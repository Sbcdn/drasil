...
use std::io::{Read,self,BufWriter,Write,BufRead}; 
...
pub fn _ccli_query_utxos_address(addr: &caddr::Address, network : String) -> Option<TransactionUnspentOutputs> {
        ...
        for line in lines_iter {
            if &line[0..1] != " " && &line[0..1] != "-" {
                ...
                let tx_hash : ccrypto::TransactionHash = match cols.next() {
                    Some(hash) => {ccrypto::TransactionHash::from_bytes(hex::decode(hash).unwrap()).unwrap()}
                    None => { panic!("TxHash expected!") }
                };
                let tx_index : u32 = match cols.next() {
                    Some(index) => {index.parse::<u32>().unwrap()}
                    None => { panic!("TxIndex (u32) expected!") }
                };
                let lovelace : cutils::BigNum = match cols.next() {
                    Some(amount) => {cutils::BigNum::from_str(amount).unwrap()}
                    None => { panic!("Lovelace value expected!") }
                };
                                    ...
                                    match elem.parse::<u64>() {
                                        ...
                                        Err(_) => {
                                            if next == "token"{
                                                ...
                                                let policy = clib::PolicyID::from_bytes(hex::decode(t.next().unwrap()).unwrap()).unwrap();
                                                let asset_name = clib::AssetName::new(hex::decode(t.next().unwrap()).unwrap()).unwrap();
                                                asset.insert(&asset_name, &cutils::to_bignum(amount));

                                                match ma.get(&policy) {
                                                    Some (p) => {
                                                        match p.get(&asset_name) {
                                                            Some (a) => {
                                                                ma.get(&policy).unwrap().insert(&asset_name, &a.checked_add(&cutils::to_bignum(amount)).unwrap());
                                                            }
                                                            None => { 
                                                ...
                                                match ccrypto::DataHash::from_bytes(hex::decode(elem).unwrap()) {
                                                    Ok(dh) => {
                                                        datumhash = Some(dh);
                                                    }
...
pub fn get_nfts_for_sale(token_utxos : &TransactionUnspentOutputs) -> Vec::<(ccrypto::ScriptHash,clib::AssetName, cutils::BigNum)> {
    ...
    for i in 0..token_utxos.len() {
        ...
        for p in 0..value.multiasset().unwrap().keys().len() {
            ...
            for a in 0..assets.len() {
                ...
                ret.push((policy_id.clone(), tn.clone(), assets.get(&tn).unwrap().clone()))
            }
        }
    }
...
pub fn find_token_in_utxo(utxo : &TransactionUnspentOutput, cs : &ccrypto::ScriptHash , tn : &clib::AssetName) -> Option<usize> {
...
pub fn _query_utxos_by_address_from_cli( addresses : Vec::<caddr::Address>) -> TransactionUnspentOutputs {
    let mut txuos = TransactionUnspentOutputs::new();
...
//Todo: Return Result 
pub fn get_smart_contract(sc : &Option<String>) -> Result<cardano_serialization_lib::plutus::PlutusScript,MurinError> {
    match sc { 
        Some(sc)  => {
...
pub fn get_input_position(inputs : clib::TransactionInputs, elem : TransactionUnspentOutput) -> (usize,Vec::<ccrypto::TransactionHash>) {
    ...
    for i  in 0..inputs.len(){
        ...
        if inputs.get(i).transaction_id() == elem.input().transaction_id() && 
            ...
            index = i;
            debug!("Found Script Input: {:?} at position : {:?}\n",inputs.get(i),index);
        }
    }
...
pub fn get_vkey_count(txuos : &TransactionUnspentOutputs, col : Option<&TransactionUnspentOutput>) -> usize {
...
pub fn make_dummy_vkeywitnesses(vkey_count : usize) -> ccrypto::Vkeywitnesses {
    ...
    let vkeywitness = ccrypto::Vkeywitness::from_bytes(hex::decode(DUMMY_VKEYWITNESS).unwrap()).unwrap();
...
pub fn get_stake_address(addr : &caddr::Address) -> ccrypto::Ed25519KeyHash {
    debug!("Address in get_stake_address: {:?}",hex::encode(addr.to_bytes()));
    ...
    match address {
        ...
        None => {
            ...
            match payment_cred_key_.kind() {
                ...
                _ => { panic!("get_stake_address discovered wrong KeyHash Type"); }
            }
        ...
        None => {
            ...
            match payment_cred_key_.kind() {
                caddr::StakeCredKind::Key => {
                    let cred_key_ = payment_cred_key_.to_keyhash().unwrap();
                    let scripthash_bytes = cred_key_.to_bytes();
                    payment_cred_key = ccrypto::Ed25519KeyHash::from_bytes(scripthash_bytes).unwrap();
                ...
                _ => { panic!("get_payment_address discovered wrong KeyHash Type"); }
            }
...
pub fn make_cardano_cli_tx(tx : String, tx_hash : String, submit : String, node_ok : bool) -> () {
    let cli_tx = SmartContract {r#type : "Tx AlonzoEra".to_string(), description : "artifct CLI transaction".to_string(), cborHex:tx};
    ...
    if submit != "false" && node_ok{
        ...
        if ret.status.success() {
            ...
            let txh = TxHash {tx_hash : tx_hash.clone(), message :  String::from_utf8(ret.stderr).unwrap()};
            serde_json::to_writer(&io::stdout(),&txh).unwrap();   
        }
...
pub fn _api_sign_tx(message : clib::TransactionBody) -> ccrypto::Vkeywitness {
    ...
    let api_priv_key = ccrypto::PrivateKey::from_extended_bytes(&hex::decode("_API_SKEY").unwrap()).unwrap();
...
pub fn make_script_outputs(tx : &mut ctxb::TransactionBuilder, datum : &ccrypto::DataHash, script_outputs : &Vec<ScriptOutput>, sc_addr : String) -> Option<(cutils::Value,TransactionUnspentOutput)> {
    //////////////////////////////////////////////////////////////////////////////////////////////////////
    //
    //Add Script Outputs
        ...
        for o in script_outputs {
            ...
            for v in &o.value {
                if v.currencySymbol == "" || v.currencySymbol == "lovelace" {
                    ...
                    let cs : clib::PolicyID  = clib::PolicyID::from_bytes(hex::decode(&v.currencySymbol).unwrap()).unwrap();
                    let mut assets = clib::Assets::new();
                    for a in &v.assets {    
                        let tn : clib::AssetName = clib::AssetName::new(hex::decode(&a.tokenName).unwrap()).unwrap();
            ...
            let txin  = clib::TransactionInput::new(&ccrypto::TransactionHash::from_bytes(hex::decode(o.txhash.clone()).unwrap()).unwrap(),o.txinput);
...
pub fn _make_wallet_outputs(tx : &mut ctxb::TransactionBuilder, outputs : &Vec<TxOutput> , mut manual_fee : bool) -> u32 {
    //////////////////////////////////////////////////////////////////////////////////////////////////////
    //
    //Add wallet outputs
...
pub fn make_script_outputs_txb(txos : &mut clib::TransactionOutputs, receiver : String , datum : &ccrypto::DataHash, script_outputs : &Vec<ScriptOutput>, set_datum: bool) -> Option<cutils::TransactionUnspentOutput> {
    //////////////////////////////////////////////////////////////////////////////////////////////////////
    //
    ...
    if script_outputs.len() == 1 {
        for o in script_outputs {
            ...
            if set_datum { txout.set_data_hash(&datum) };
            txos.add(&txout);
...
pub fn splitt_ada_off(value : cutils::Value , addr : caddr::Address , split_txos : &mut clib::TransactionOutputs) {
    let min_utxo_ada = txbuilders::calc_min_ada_for_utxo(&value, None);
    let coins = value.coin();
    match coins.compare(&min_utxo_ada.checked_add(&cutils::to_bignum(MIN_ADA*5)).unwrap()) {
        1 => {
                        ...
                        if txo_size < B_MAX {
                            ...
                            match multi.keys().len() {
                                ...
                                1 => {
                                    ...
                                    match assets.len() {
                                        ...
                                        _ => {
                                            ...
                                            let mut new_value = cutils::Value::new(&cutils::to_bignum(2000000));
                                            let mut new_multi = clib::MultiAsset::new();
                                            new_multi.insert(cs, &new_asset);
                                            new_value.set_multiasset(&new_multi);
                                            let min_utxo_new_val = txbuilders::calc_min_ada_for_utxo(&new_value, None);
                                            new_value.set_coin(&min_utxo_new_val);
                                            
                                            // Value for Recursive call
                                            let coin_dif = value.coin().checked_sub(&min_utxo_new_val).unwrap();
                                            ...
                                            let min_utxo = txbuilders::calc_min_ada_for_utxo(&rest_value, None);
                                            let rest_txo = clib::TransactionOutput::new(&addr, &rest_value);

                                            if coin_dif.compare(&min_utxo) == -1 {
                                                // Not enough Ada to cover min utxo on a split leave the utxo as it is
                                                split_txos.add(&txo);
                                            } else {
                                                let nex_txo = &clib::TransactionOutput::new(&addr, &new_value);
                                ...
                                _ => {
                                    ...
                                    let mut worker_val = cutils::Value::new(&cutils::to_bignum(2000000u64));
                                    let mut new_txo = clib::TransactionOutput::new(&addr, &worker_val);
                                        ...
                                        match work_multi.get(&work_multi.keys().get(i)) {
                                            ...
                                            Some (assets) => {
                                                if assets.len() <= 0 || j >= assets.len() {
                                                    ...
                                                    let mut new_multi = clib::MultiAsset::new();
                                                    new_multi.insert(&work_multi.keys().get(i), &new_token);
                                                    let mut temp_val = cutils::Value::new(&cutils::to_bignum(0u64));
                                                    temp_val.set_multiasset(&new_multi);
                                                    worker_val = worker_val.checked_add(&temp_val).unwrap();
                                                    j += 1;
                                                }
                                                worker_val.set_coin(&txbuilders::calc_min_ada_for_utxo(&worker_val, None));
                                                let dtxo = clib::TransactionOutput::new(&addr, &worker_val);
                                    ...
                                    match value.coin().checked_sub(&new_txo.amount().coin().checked_add(&cutils::to_bignum(MIN_ADA)).unwrap()) {
                                        Ok(_) => {
                                            debug!("here I am");
                                            let rest_val = value.checked_sub(&new_txo.amount()).unwrap();
                                            match rest_val.multiasset() {
                                                None => {
                                                    ...
                                                    new_txo = clib::TransactionOutput::new(&addr, &added_val);
                                                }

                                                Some (multi) => {
                                                    debug!("Bla: {:?}",multi.get(&multi.keys().get(0)).unwrap().len());
                                                    if multi.len() == 0 || multi.get(&multi.keys().get(0)).unwrap().len() == 0 {
                                            ...
                                            split_output_txo(clib::TransactionOutput::new(&addr, &rest_val), split_txos);
                                        }
...
pub fn splitt_coin_multi(txins : &TransactionUnspentOutputs) -> (TransactionUnspentOutputs, TransactionUnspentOutputs) {
...
pub fn find_suitable_coins(nv : &mut cutils::Value, inputs : &mut TransactionUnspentOutputs, overhead: u64) -> (Option<TransactionUnspentOutputs>,u64) {
    ...
    'outer: for tx in inputs.clone() {
        debug!("\n-------TXIn : {:?}{:?}",tx.input().transaction_id(),tx.input().index());

        let lc = cutils::from_bignum(&tx.output().amount().coin());
        debug!("\n---LC: {:?}----TXIn : {:?}{:?}",lc,tx.input().transaction_id(),tx.input().index());
        if  lc > coins {
            match tx.output().amount().multiasset() {
                Some (multi) => {
                    match multi.len() { 
    ...
    if coin_storage.len() > 0 {
        ...
        for tx in inputs {
            debug!("\n-------TXIn in Acc : {:?}{:?}",tx.input().transaction_id(),tx.input().index());
            let lc = cutils::from_bignum(&tx.output().amount().coin());
            acc = acc +lc;
            debug!("Acc {:?}, LC: {:?}",acc,lc);
            selection.add(&tx);
...
pub fn find_asset_utxo(txuos : TransactionUnspentOutputs, nft_cs : ccrypto::ScriptHash , nft_tn : clib::AssetName) -> Option<usize> {
    if txuos.len() > 0 {
        debug!("len > 1:  {:?}",txuos);
            ...
            match value.multiasset() {
                Some(multi) => {
                    match multi.get(&nft_cs) {
                        ...
                        None        => { debug!("No Assets found for Policy ID:{:?}  found",nft_cs); 
                                            debug!("Multi: {:?}\n",multi);
                    }
                        
                    }
                }
                None        => { debug!("No Policy Id found");}
            }        
        }
...
pub fn find_asset_utxos_in_txuos(txuos : &TransactionUnspentOutputs, listing_tokens : &Vec::<(ccrypto::ScriptHash,clib::AssetName, cutils::BigNum)>) -> Vec::<usize> {
                ...
                match value.multiasset() {
                    Some(multi) => {
                        match multi.get(&cs) {
                            ...
                            None        => { debug!("No Assets found for Policy ID:{:?}  found",cs); 
                                                debug!("Multi: {:?}\n",multi);
                        }
...
pub fn find_collateral_by_txhash_txix(elem : &TransactionUnspentOutput, txuos: &TransactionUnspentOutputs) -> Option<usize> {
...
pub fn find_utxos_by_address(addr : caddr::Address, txuos : &TransactionUnspentOutputs) -> (TransactionUnspentOutputs,TransactionUnspentOutputs) {
    ...
    collateral : Option<cutils::TransactionUnspentOutput>, 
        ...
        if let Some (token_utxos) = token_utxos {
            for i in 0..token_utxos.len() {
                        ...
                        acc = acc.checked_add(&token_utxos.get(i).output().amount()).unwrap();
                        nv = nv.checked_add(&token_utxos.get(i).output().amount()).unwrap();
                        ...
                        if let Some(i) = multiassets.find_utxo_index(&token_utxos.get(i)) {
                            ...
                            debug!("Deleted token utxo from multiasset inputs: \n {:?}\n",tutxo);    
                        }    
            }
        }
        ...
        if let Some(cutxo) = collateral {
                ...
                debug!("Some collateral to check for deletion found, Index: {:?}",c_index);
        ...
        debug! ("\n\nNV: {:?}",nv);
        debug! ("\n\nNV: {:?}",acc);
        debug!("\nbefore while! Utxo Count: {:?}, {:?} \n", utxo_count, (nv.coin().compare(&acc.coin()) > 0));
   ...
   // script_outputs : Option<&TransactionUnspentOutput>,
    ...
    dummyrun : &bool,
            ...
            if txos.len() > 0 {
                for i in 0..txos.len() {               
                    *tbb_values = tbb_values.checked_add(&txos.get(i).amount()).unwrap();
                    debug!("\nAdding value from existing outputs to to_be_paid: {:?}",tbb_values.coin());                            
                }
            }
        ...
        match option_next_input {
            Some(unspent_output) => { // cutils::TransactionUnspentOutput {input, output}     
                let value = unspent_output.output().amount();
                ...
                debug!("\nInputs Address: {:?}",hex::encode(input_address_pkey.to_bytes()));
                ...
                return balance_tx(input_txuos, tokens, txos, already_paid, &fee, fee_paied, 
                                    first_run, txos_paied, tbb_values, senders_addr, change_address, acc_change, sc_addr, dummyrun);
            } 
            None => {
                ...
                if *txos_paied && *fee_paied && acc_change.coin().compare(&min_ada_value.coin()) >= 0  {
                    ...
                    *acc_change = acc_change.checked_sub(&acc_change).unwrap();
                    ...
                    if (acc_change.coin().compare(&min_utxo) == -1 &&
                        !(acc_change.coin().compare(&cutils::to_bignum(0u64)) == 0))  && !*dummyrun    
                    {
                        panic!("\nERROR: Transaction does balance but last output is below min Ada value: {:?} overhead",acc_change.coin());
                        
                    }
                }
                if (!*txos_paied || !*fee_paied || acc_change.coin().compare(&cutils::to_bignum(0u64)) != 0 || acc_change.multiasset() != None ) && !*dummyrun {
...
pub fn make_inputs_txb(inputs : &Vec<TxInput>) -> (clib::TransactionInputs, TransactionUnspentOutputs) {
    ...
    let mut txins = clib::TransactionInputs::new();
    for i in inputs {
        let txuo_in = &clib::TransactionInput::new(&ccrypto::TransactionHash::from_bytes(hex::decode(i.txhash.clone()).unwrap()).unwrap(),i.txinput);
        ...
        let txuo : cutils::TransactionUnspentOutput = cutils::TransactionUnspentOutput::new(&txuo_in,&txuo_out);
        //info!("TXOU: {:?}",txuo);
        //info!();
...
pub fn make_datum_mp(
    ...
    let pub_key_hash = hex::encode(
            wallet_address.payment_cred().to_keyhash().unwrap().to_bytes()
        );
    let stake_key_hash = hex::encode(
        wallet_address.stake_cred().to_keyhash().unwrap().to_bech32("addr_stake").unwrap().as_bytes()
    );
    debug!("PubKeyHash: {:?}, StakeKeyHash: {:?}\n",pub_key_hash,stake_key_hash);
    ...
    fields_inner.add(&plutus::PlutusData::new_integer(&cutils::BigInt::from_str(selling_price).unwrap()));              // Selling Price
    fields_inner.add(&plutus::PlutusData::new_bytes(hex::decode(pub_key_hash.clone()).unwrap()));                       // Sellers PubKeyHash
    fields_inner.add(&plutus::PlutusData::new_integer(&cutils::BigInt::from_str(&royalties_rate).unwrap()));            // royalties rate in promille
    fields_inner.add(&plutus::PlutusData::new_bytes(roy_pkey_hash.clone()));                                            // Royalties PubKeyHash
    fields_inner.add(&plutus::PlutusData::new_bytes(hex::decode(policy_id).unwrap()));                                  // PolicyId
    fields_inner.add(&plutus::PlutusData::new_bytes(hex::decode(token_name).unwrap()));                                 // TokenName
    ...
    meta_list.push(format!("{}{}","artifct-cli-",sc_version).as_bytes().to_vec());
...
pub fn decode_datum_mp(metadata_datum : Vec<String>, net : &clib::NetworkIdKind) 
    ->  Result<(cutils::BigNum,String,cutils::BigNum,clib::PolicyID,String,plutus::PlutusList,ccrypto::DataHash,ccrypto::Ed25519KeyHash,String),MurinError> { //(ccrypto::DataHash, plutus::PlutusList) {
    ...
    let stake_str =  format!("{}{}",str::from_utf8(&m4.clone())?,str::from_utf8(&m5.clone())?);
    ...
    info!("\nDecode Datum: price: {:?}, PubKey Owner: {:?}, RR: {:?}, Policy: {:?}, TN: {:?}\n",selling_price,trade_owner,royalties_rate,nft_policy_id,hex::encode(m3.clone())); // , m0,m1,m2,m3,m4); // 

    let mut fields_inner = plutus::PlutusList::new();
    fields_inner.add(&plutus::PlutusData::new_integer(&cutils::BigInt::from_str(&m0)?));          // Selling Price
    fields_inner.add(&plutus::PlutusData::new_bytes(m1.clone()));                                         // Sellers PubKeyHash
    fields_inner.add(&plutus::PlutusData::new_integer(&cutils::BigInt::from_str(&rr)?));          // Royalties Rate
    ...
    Ok((selling_price,trade_owner,royalties_rate,nft_policy_id,hex::encode(m3.clone()),datums,datumhash,royalties_pkey,m6))
}
...
pub fn calc_txfee(
    ...
    {
    let txsfee = tx_script_fee(ex_unit_price,cutils::from_bignum(&steps),cutils::from_bignum(&mem));
...
pub fn tx_script_fee(ex_unit_price: ExUnitPrice , steps : u64 , mem :u64) -> u64 {
    ...
    let tx_script_fee = (ex_unit_price.priceMemory * mem as f64) + (ex_unit_price.priceSteps * steps as f64);
...
pub fn create_ada_tx( 
 ...
 ) -> Result<(clib::TransactionBody, clib::TransactionWitnessSet, clib::metadata::AuxiliaryData, usize, TransactionUnspentOutputs),MurinError> {
    ...
    let security = cutils::to_bignum(cutils::from_bignum(&needed_value.coin()) / 100 * 10 + MIN_ADA); // 10% Security for min utxo etc. 
