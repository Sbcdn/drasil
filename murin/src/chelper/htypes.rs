...
impl TransactionUnspentOutputs {
    ...
    pub fn find_utxo_index(&self, elem : &TransactionUnspentOutput) -> Option<usize> {
        ...
        for i in 0..self.0.len() {
            ...
            if txi.transaction_id().to_bytes() == elem_hash.to_bytes() && txi.index() == elem_index{
                debug!("Found utxo by txhash: {:?}, txix: {:?}",elem_hash,elem_index);
                    return Some(i)   
            }
        }
        return None
    ...
    pub fn sort_by_coin(&mut self) -> () {
        self.0.sort_by_cached_key(|k| cutils::from_bignum(&k.output().amount().coin()));
    }
    ...
    pub fn sort_by_asset_amount(&mut self, policy: &ccrypto::ScriptHash, asset: &clib::AssetName) -> () {
        ...
        fn get_amount(k : &TransactionUnspentOutput , policy: &ccrypto::ScriptHash, asset: &clib::AssetName) -> usize {
                ...
                match diff.len() {
                    ...
                    if let Some(k) = list_of_utxo_tokens.iter().find(|k| k.0==diff ){
                            ...
                            if let Some (pos) = self.find_utxo_index(&k.1){
                                ...
                                overhead_tok = value_to_tokens(&tot_val_utxos.clamped_sub(&needed_val))?;
                            }
                    }
                    },
    ...
    pub fn coin_sum(&self) -> u64 { //cutils::BigNum {
        let coinsum = self.0.iter().fold(0u64,|acc : u64 ,x| acc + cutils::from_bignum(&x.output().amount().coin()));
        //cutils::to_bignum(coinsum)
        coinsum
    }

    pub fn coin_sum_minutxo(&self) -> u64 { //cutils::BigNum {
        let coinsum = self.0.iter().fold(0u64,|acc : u64 ,x| acc + cutils::from_bignum(&x.output().amount().coin()));
        let minutxo = cutils::from_bignum(&crate::calc_min_ada_for_utxo(&self.calc_total_value().expect("calc total value crashed in coin_sum_minutxo"), None));

        std::cmp::min(coinsum as i64 - minutxo as i64,0) as u64
    }

    pub fn coin_value_subset(&self, ncoin : cutils::BigNum, already_in_use: Option<&TransactionUnspentOutputs>) -> TransactionUnspentOutputs {
        use itertools::FoldWhile::{Continue, Done};
        let i = self.0.iter().fold_while(TransactionUnspentOutputs::new(),|mut acc : TransactionUnspentOutputs, x| { 
            if acc.coin_sum() >= cutils::from_bignum(&ncoin) { Done(acc) } else {
                Continue( {
    ...
    pub fn coin_value_subset_minutxo(&self, ncoin : &cutils::BigNum, payaddr : &caddr::Address) -> TransactionUnspentOutputs {
        use itertools::FoldWhile::{Continue, Done};
        let i = self.0.iter().fold_while(TransactionUnspentOutputs::new(),|mut acc : TransactionUnspentOutputs, x| { 
            if acc.coin_sum_minutxo() >= cutils::from_bignum(&ncoin) { Done(acc) } else {
                Continue( {
                    let x_stake = crate::cip30::get_stake_address(&x.output().address()).expect("Could not determine stake address in coin_value_subset_minutxo 1");
                    let payer_stake = crate::cip30::get_stake_address(payaddr).expect("Could not determine stake address in coin_value_subset_minutxo 2");
    ...
    pub fn get_coin_only(&self) -> TransactionUnspentOutputs {
        let coin_only = self.0.iter().filter(|n| n.to_owned().output().amount().multiasset().is_none()).collect();
        coin_only
    }
    ...
    pub fn contains_any(&self, elems : &TransactionUnspentOutputs) -> bool {
        for i in 0..self.0.len() {
            ...
            for j in 0..elems.len() {
                ...
                if txi_self.transaction_id().to_bytes() == txi_other.transaction_id().to_bytes() && txi_self.index() == txi_other.index() {
    ...
    pub fn from_hex(str : &str) -> Result<TransactionUnspentOutputs,crate::MurinError> {
        ...
        (|| -> Result <(),crate::MurinError> {
            let len = raw.array()?;
            while match len { cbor_event::Len::Len(n) => v.len() < n as usize, cbor_event::Len::Indefinite => true,} {
    ...
    pub fn find_utxo_by_txhash(&self, tx_hash: &String, tx_index: u32) -> Option<usize> {
        for (i,e) in self.0.clone().into_iter().enumerate() {
            if hex::encode(e.input().transaction_id().to_bytes()) == *tx_hash && e.input().index() == tx_index {
                return Some(i)
            } 
        }
        None
...
pub fn value_to_tokens(value: &cutils::Value) -> Result<Tokens,MurinError> {
    let ma = match value.multiasset() {
        ...
        None => return Err(MurinError::new("Value does not contain any multiassets cannot convert to 'Tokens'"))
    };
...
pub fn find_tokenindex_by_policy_assetname(t: &Tokens, p: &ccrypto::ScriptHash, a: &clib::AssetName) -> Option<usize> {
...
pub fn check_overhead_tokens(tok_l: &Tokens, tok_r: &Tokens) -> Tokens {
    ...
    for tok in tok_l {
        if let Some(s) = find_tokenindex_by_policy_assetname(&tok_r, &tok.0,&tok.1) {
            if tok_r[s].2.compare(&tok.2) >= 0 {   //cutils::from_bignum(&r) >= 0 {
