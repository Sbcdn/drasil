...
pub fn perform_rwd ( 
    ...
    dummy       : bool
        ...
        for dat in raw_metadata {
            ...
            metalist.add(&clib::metadata::TransactionMetadatum::new_bytes(dat.as_bytes().to_vec())?);
        }
        metalist.add(&clib::metadata::TransactionMetadatum::new_bytes(rwdtxd.get_stake_addr().to_bytes())?);
        metalist.add(&clib::metadata::TransactionMetadatum::new_bytes(recipient_address_bech32_1.as_bytes().to_vec())?);
        metalist.add(&clib::metadata::TransactionMetadatum::new_bytes(recipient_address_bech32_2.as_bytes().to_vec())?);
        metalist.add(&clib::metadata::TransactionMetadatum::new_bytes(ns_version.as_bytes().to_vec())?);

        let metadata = clib::metadata::TransactionMetadatum::new_list(&metalist);
        general_metadata.insert(
            &cutils::to_bignum(631u64),
            &metadata
        );
        ...
        if ! trade_owner.contains(&recipient_address) {
            return Err(MurinError::new("Error: RewardAddress is not contained in input addresses, this is mandatory"));
        }
        ...
        // Reward return Value 
        let mut rwd_utxo_selection = find_token_utxos_na(&rwd_utxos_avail, rwdtxd.get_reward_tokens(), Some(native_script_address.clone()).as_ref())?;
        let rwd_utxo_tot_val   = rwd_utxo_selection.calc_total_value()?;
        debug!("Selected Inputs Reward UTXOs Total Value: {:?}",rwd_utxo_tot_val);
        ...
        if rwd_utxo_selection.len() > 0 {
            ...
            if return_val.multiasset().is_some() {
                ...
                if let Some(more_ada_needed) = return_values.1 {
                    additional_inputs = rwd_coin_only.coin_value_subset(more_ada_needed, Some(&rwd_utxo_selection));
                    ...
                    if more_ada_needed.compare(&additional_inputs.calc_total_value()?.coin()) >= 0 {
                        ...
                        txouts.add(&clib::TransactionOutput::new(&native_script_address, &return_val)); 
                    }
            ...
            return Err(MurinError::new("ERROR: No Utxos for Reward Distribution available"));
        }       
        ...
        let security = cutils::to_bignum(cutils::from_bignum(&needed_value.coin()) / 100 * 10 + (2*htypes::MIN_ADA)); // 10% Security for min utxo etc. 
        ...
        let slot = cutils::to_bignum(gtxd.clone().get_current_slot() + hfn::get_ttl_tx(&gtxd.clone().get_network()));
...
pub async fn build_rwd_multisig (
            ...
            let ex_unit_price : htypes::ExUnitPrice = crate::ExUnitPrice { priceSteps: 7.21e-5, priceMemory:  5.77e-2 };
        ...
        //Create Tx
            let (txbody_,mut txwitness_, aux_data_, _,vkey_counter) = 
                perform_rwd(&cutils::to_bignum(2000000), ns_addr, ns_script, ns_version, gtxd, rwdtxd, pvks, apply_system_fee,true)?;
            ...
            let calculated_fee = hfn::calc_txfee (&transaction_,&a,&b,ex_unit_price.clone(),&steps,&mem,true);   
            let (txbody, txwitness, aux_data,used_utxos,vkey_counter_2) = 
                perform_rwd(&calculated_fee, ns_addr, ns_script, ns_version, gtxd, rwdtxd, &pvks, apply_system_fee, false)?;          
            ...
            if vkey_counter_2 != vkey_counter || transaction2.to_bytes().len() != transaction_.to_bytes().len() {
                ...
                let calculated_fee = hfn::calc_txfee (&transaction2,&a,&b,ex_unit_price,&steps,&mem,true);   
                let (txbody, txwitness, aux_data,used_utxos,_) = 
                    perform_rwd(&calculated_fee, ns_addr, ns_script, ns_version, gtxd, rwdtxd, &pvks, apply_system_fee,false)?;     
                info!("Fee: {:?}",calculated_fee);
                Ok(hfn::tx_output_data (txbody,txwitness,aux_data,used_utxos.to_hex()?,0u64,false)?)
            } else {
            info!("Fee: {:?}",calculated_fee);
                Ok(hfn::tx_output_data(txbody,txwitness,aux_data,used_utxos.to_hex()?,0u64,false)?)
            }            
