...
impl ToString for MpTxData {
    fn to_string(&self) -> String {
        ...
        match  self.get_metadata() {
            ...
            None => {s_metadata = "NoData".to_string();}
        }
    ...
    fn from_str(src: &str) -> Result<Self, Self::Err> {
        ...
        if slice.len() == 6 { 
            ...
            let token_utxos =  match slice[1] {
                "NoData" => return Err(MurinError::new("Error: No Tokens for sale in marketplace transaction data")),
            ...
            Err(
                ...
                //    std::io::ErrorKind::InvalidData,
                    &format!("Error the provided string '{}' cannot be parsed into 'MpTxData' ",src).to_string()
                )
            )
        }
    }
...
impl MpTxData {
    ...
    pub fn new(tokens : Vec::<TokenAsset>, token_utxos: TransactionUnspentOutputs, selling_price : u64) -> MpTxData {
        MpTxData { 
            tokens: tokens, 
            token_utxos: token_utxos, 
            royalties_addr: None, 
