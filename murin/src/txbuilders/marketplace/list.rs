...
pub fn perform_listing ( 
    ...
    dummy       : bool
        ...
        let datumpair = hfn::make_datum_mp(
                                    ...
                                    &gtxd.clone().get_senders_addresses()[0].clone().to_bech32(None).unwrap(), 
                                    &roy_rate.to_string(),
...
pub async fn build_mp_listing (gtxd : &super::TxData , mptxd : &super::MpTxData, sc_addr : &String, sc_version : &String) -> Result<htypes::BuildOutput, MurinError> {
...
//Create Tx
    let (txbody_,mut txwitness_, aux_data_, used_utxos,vkey_counter) = 
            perform_listing(&cutils::to_bignum(2000000), sc_addr, sc_version, gtxd, mptxd, true)?;
