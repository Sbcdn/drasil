...
pub fn perform_update( 
 ...
 ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        let datumpair_new = hfn::make_datum_mp(
            &mptxd.selling_price.to_string(), 
            &decoded_datum.1, 
            &decoded_datum.2.to_str(),
            &hex::encode(decoded_datum.3.to_bytes()), 
            &decoded_datum.4,
            &decoded_datum.7,
            &decoded_datum.8);
        ...
        let datum = datumpair_new.2; //.get(0).as_constr_plutus_data().unwrap().data();
        ...
        for dat in 0..datum.len() {
            ...
            metalist.add(&clib::metadata::TransactionMetadatum::new_bytes(datum.get(dat).unwrap().to_vec()).unwrap());
        }
        let metadata = clib::metadata::TransactionMetadatum::new_list(&metalist);
        general_metadata.insert(
            &cutils::to_bignum(312u64),
            &metadata
        );
        ...
        if input_txuos.contains_any(&signers_address_utxos) {
            ...
            if !signers_address_utxos.is_empty() {
                ...
                let transaction_ = clib::Transaction::new(&internal_transfer.0,&internal_transfer.1,Some(internal_transfer.2.clone()));
                let calculated_fee = hfn::calc_txfee(&transaction_,&a,&b,ex_unit_price,&steps,&mem,true);   
                ...
                hfn::tx_output_data(txbody, txwitness, aux_data, used_utxos.to_hex()?, 0u64, true)?;
                std::process::exit(0);
            }
        }
        ...
        debug!("Script Hash Data Concat: {:?}",hex::encode(scriptdatahash.to_bytes()));
...
pub async fn build_mp_update (gtxd : &super::TxData , mptxd : &super::MpTxData, sc_script: &String, sc_addr : &String) -> Result<htypes::BuildOutput, MurinError> {
    ...
    let (txbody_,mut txwitness_, aux_data_, _,vkey_counter) = 
            perform_update(&cutils::to_bignum(2000000), sc_script, sc_addr, gtxd, mptxd, true)?;
