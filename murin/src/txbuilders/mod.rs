...
use crate::{TransactionUnspentOutputs, TransactionUnspentOutput};
...
impl TxData {
    pub fn new(
        ...
        let sa = match sstake {
            Some(stake) => stake,
            None => {
                        crate::cip30::get_reward_address(&saddresses[0])?   
            }
        };
           Ok(TxData {
            user_id : None,
...
impl ToString for TxData {
    fn to_string(&self) -> String {
        ...
        match self.get_outputs() {
            Some(o) => if let Ok(ok) = o.to_hex() { s_outputs = ok },
        ...
        match self.get_excludes() {
            Some(ex) => if let Ok(ok) = ex.to_hex() { s_excludes = ok },
            _ => {s_excludes = "NoData".to_string()}
        }
        
        //prepare collateral
        let s_collateral = 
        match self.get_collateral() {
    ...
    fn from_str(src: &str) -> Result<Self, Self::Err> {
        ...
        if slice.len() == 10 { 
            ...
            let collateral = match slice[5] {
                "NoData" => None,
                _        => Some(TransactionUnspentOutput::from_bytes(hex::decode(slice[5])?)?)
            };
            ...
            let network = match slice[6] {
                ...
                _         => { return Err(MurinError::new(&"ERROR network could not be restored from string")) }
            };
            ...
            Err(
                MurinError::new(
                    &format!("Error the provided string '{}' cannot be parsed into 'TxData' ",src).to_string()
                )
            )
        }
    }
...
pub async fn find_token_utxos(inputs : TransactionUnspentOutputs, assets: Vec::<TokenAsset>) -> Result<TransactionUnspentOutputs, MurinError> {
    ...
    if !inputs.is_empty() && !assets.is_empty() {
            ...
            for i in 0..inputs.len() {
                ...
                if let Some(multi) = value.multiasset() {
                    if let Some(toks ) = multi.get(&asset.0) {
                       if let Some(amt) =  toks.get(&asset.1) { // for tok in 0..toks.len()  {
                            if needed_amt >= asset.2 {
                                log::debug!("Found a utxo containing {} tokens {}.{}!",asset.2.to_str(),hex::encode(asset.0.to_bytes()),hex::encode(asset.1.to_bytes()));
       ...
       return Err(MurinError::new("cannot find token utxos , one of the provided inputs is empty"));
    }
    Ok(out)
}

pub fn find_token_utxos_na(inputs : &TransactionUnspentOutputs, assets: Vec::<TokenAsset>,  on_addr : Option<&caddr::Address>) -> Result<TransactionUnspentOutputs, MurinError> {
    let mut out = TransactionUnspentOutputs::new();
    ...
    if !inputs.is_empty() && !assets.is_empty() {
       ...
       return Err(MurinError::new("ERROR: cannot find token utxos , one of the provided inputs is empty"));
    }

   
    if out.len() <= 0 {
        debug!("Inputs: {:?}",inputs);
        return Err(MurinError::new("ERROR: The token is not available in the utxo set"));
    }
...
pub fn input_selection(
    ...
    on_addr : Option<&caddr::Address>,
        ...
        if let Some(cutxo) = collateral {
                debug!("Col: {:?}",cutxo);
                let c_index = crate::chelper::hfn::find_collateral_by_txhash_txix(&cutxo,&purecoinassets);
                ...
                if let Some(index) = c_index {
                        ...
                        // Double check
                        if let Some (_)= crate::chelper::hfn::find_collateral_by_txhash_txix(&cutxo,&purecoinassets) {
                                return Err(MurinError::new("PANIC COLLATERAL COULDN'T BE EXCLUDED FROM SELECTION SET"));
                        }  
                }  
        }
        ...
        if needed_value.multiasset().is_some() {
            ...
            if token_selection.len() > 0 {
                for i in 0..token_selection.len() {
                            ...
                            acc = acc.checked_add(&token_selection.get(i).output().amount()).unwrap();
                            debug!("\n\nAdded Script Utxo to Acc Value : \n {:?}\n",acc);
                            // Delete script input from multi assets
        ...
        while nv.coin().compare(&acc.coin()) > 0 && max_run < utxo_count {
            ...
            if purecoinassets.len() <= 0 {
                ...
                let ret = crate::chelper::hfn::find_suitable_coins(&mut nv,&mut purecoinassets,overhead); 
                ...
                match ret.0 {
                    ...
                    None => {
                        return Err(MurinError::new("ERROR: Not enough input utxos available to balance the transaction"));
                    }
                }
...
impl Persona {
    ...
    fn add_left(&mut self, other: &Self) -> Result<(),MurinError> {
        if let Some(o_txuo_out) = &other.txuo_out {
            if let Some(s_txuo_out) = &self.txuo_out {
                ...
                self.txuo_out = Some(clib::TransactionOutput::new(&s_txuo_out.address(), &new_val));
            } else {
    ...
    pub fn addl(&mut self, other: &Self) -> Result<(),MurinError> {
        if !self.ceq(other){
            return Err(MurinError::new("Payment Combination Error: The two Persona do not are not the same"))
        } 
        self.add_left(other)?;
        Ok(())
    }
    ...
    pub fn set_output_value(&mut self, v: cutils::Value, hash: Option<ccrypto::DataHash>) -> Result<(),MurinError> {
    ...
    pub fn set_min_utxo_value(&mut self) -> Option<cutils::BigNum> {
        ...
        if self.get_output_coins().compare(&c_min) == (-1) {
            self.set_output_coins(c_min).expect("Could not set coin value in set min utxo value for persona");
            return Some(c_min);
        }
        None
    }
            ...
            match needed.multiasset() {
                Some(_) => {
                    ...
                    let mut utxo_selection = find_token_utxos_na(
                        avail_input_utxos, 
                        assets, 
                        Some(payer.get_payer()).as_ref())?; // ToDo: is paying address just from script or also wallet (stake key?)
                    
                    input_utxos.delete_set(&utxo_selection);

                    let _ = select_coins(&mut utxo_selection, &mut input_utxos, &needed, &payer.get_payer(), &self.own_address)?;                    
                },
                None => {
...
fn select_min_utxo_input_coins(
    ...
    if diff.coin().compare(&change_min_utxo) == -1 {
        ...
        let additional_utxos = avail_input_utxos.coin_value_subset_minutxo(
            &missing_coins,paying_address);
        ...
        *selected_value = selected_inputs.calc_total_value()?;
        return select_min_utxo_input_coins(paying_address, needed, selected_value, selected_inputs, avail_input_utxos);
    }
    Ok(())
}
...
impl Personas {
    ...
    fn combinator<'a> (a: &'a mut Vec::<Persona>, b: &'a mut Vec<Persona>) -> (&'a Vec<Persona>, &'a Vec<Persona>) {
        match b[..] {
            [] => (a,b),

            _ => {
                b.retain(|n| *n != a[0]);
                let combinable_bs : Vec<_> = b.iter_mut().filter(|n| a[0].ceq(n)).map(|n| n.to_owned()).collect();
...
pub fn balance_transaction(
    ...
    personas.p.iter_mut().for_each(|n| match n.txo_from_payers() {
        Ok(_) => { },
        Err(_) => {
            invalid = true
        }
...
*/
    ...
    let to_big : Vec<_> = personas.p.iter().filter(|n| n.txuo_out.clone().unwrap().to_bytes().len() > 5000 ).collect();
...
pub fn half_utxo(
    ...
    let mut multiasset_two = two.multiasset().expect("Error: The TxOut to be halfed has no multiassets!");
    ...
    match two.coin().compare(&total_min_ada) {
        -1 => {
            ...
            let additional_inputs = inputs.coin_value_subset_minutxo(
                &missing_coins,&paying_address);
            ...
            tot_val = tot_val.checked_sub(&cutils::Value::new(&total_min_ada)).unwrap();
...
pub fn find_assets_in_value( v: &cutils::Value, a: &Vec::<TokenAsset>) -> (bool,cutils::Value,cutils::Value) {
...
pub fn calc_min_ada_for_utxo(value : &cutils::Value, dh : Option<ccrypto::DataHash>) -> cutils::BigNum {
    ...
    let size = bundle_size(&value, &htypes::OutputSizeConstants{k0:2, k1:6, k2:12, k3:1, _k4:8});

    let utxo_entry_size_without_val = 27u64; //29
    let min_ada = cutils::to_bignum(dhsize + utxo_entry_size_without_val + size as u64 ).checked_mul(&cutils::to_bignum(34482u64)).unwrap(); //(value.to_bytes().len() as u64)) / 2
...
pub fn bundle_size( value : &cutils::Value , osc : &htypes::OutputSizeConstants ) -> usize {
    match &value.multiasset() {
        Some(assets) => {
            ...
            osc.k1 + ( roundup_bytes_to_words(
                (num_assets * osc.k2) + anl + (osc.k3 * pil)) // + (osc.k4 - 1))/osc.k4
            )
        }
...
pub async fn create_and_submit_cbor_tx(tx : String, tx_hash : String) -> Result<String,MurinError> {
    let cli_tx = CBORTransaction {r#type : "Tx BabbageEra".to_string(), description : "drasil build this transaction for you".to_string(), cborHex:tx};
    debug!("{:?}",cli_tx);    
...
pub async fn submit_endpoint(tx: &Vec<u8> , endpoint: String, own_tx_hash: &String ,client: &reqwest::Client) -> Result<(String,String,bool),MurinError> {
    use futures::pin_mut;

    let response = client.post(endpoint.clone())
    ...
    match tokio::time::timeout(std::time::Duration::from_secs(5), &mut response).await {
        ...
        Err(_) =>{
            ...
            Ok(("".to_string(),format!("ERROR: '{:?}' is not available",endpoint),false))
        ...
        Ok(no_timeout) => {
            match no_timeout {
                ...
                Err(e) => {
                    Ok(("".to_string(),format!("ERROR: '{:?}' is not available",e),false))
                }
            }
        }
    }
...
pub async fn submit_tx(tx: &CBORTransaction, own_tx_hash: &String) -> Result<String, MurinError> {
    ...
    if response1.2 || response2.2 || response3.2 {
        ...
        Err(MurinError::new(&(response1.1+&response2.1+&response3.1).to_string()))
    }
}
...
pub fn split_value(value: cutils::Value,) -> Result<(Vec<cutils::Value>,Option<cutils::BigNum>),MurinError> {
...
pub fn minimize_coins_on_values(values: Vec<cutils::Value>) -> Result<Vec::<cutils::Value>,MurinError> {
    let mut out = Vec::<cutils::Value>::new();
    let ada = values.iter().fold(cutils::Value::new(&cutils::to_bignum(0)),|mut acc : cutils::Value, x : &cutils::Value| {
        let c = x.coin();
        let mut xc = x.clone();
