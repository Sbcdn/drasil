...
pub fn perform_mint ( 
    ...
    dummy                   : bool
        ...
        match ns_vendorscript_addr {
            Some(_) => {
                txouts.add(&clib::TransactionOutput::new(&vendor_script_address, &mint_val));
            },
            None => {
        ...
        // Add System Mint Fee
        if *apply_system_fee {
            txouts.add(&clib::TransactionOutput::new(&rwd_system_fee_wallet, &cutils::Value::new(&cutils::to_bignum(rwd_system_fee))));
        }
        ...
        log::info!("Added Slot Time: {:?}",get_ttl_tx(&gtxd.clone().get_network()));
...
pub async fn build_mint_multisig(
        ...
        //Create Tx
            let (txbody_,mut txwitness_, aux_data_, _,vkey_counter) = 
                perform_mint(&cutils::to_bignum(2000000), vendor_ns_addr, ns_script, ns_version, gtxd, minttxd, pvks, apply_system_fee,true)?;
            ...
            let (txbody, txwitness, aux_data,used_utxos,vkey_counter_2) = 
                perform_mint(&calculated_fee, vendor_ns_addr, ns_script, ns_version, gtxd, minttxd, &pvks, apply_system_fee, false)?;          
