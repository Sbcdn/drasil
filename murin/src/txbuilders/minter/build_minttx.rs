...
pub async fn build_mint_multisig(
    ...
    //Create Tx
    let (txbody_, mut txwitness_, aux_data_, _, vkey_counter) = perform_mint(
    ...
    let (txbody, txwitness, aux_data, used_utxos, vkey_counter_2) = perform_mint(
        &calculated_fee,
        vendor_ns_addr,
        ns_script,
        ns_version,
        ...
        || transaction2.to_bytes().len() != transaction_.to_bytes().len()
    {
        ...
        let (txbody, txwitness, aux_data, used_utxos, _) = perform_mint(
            &calculated_fee,
            vendor_ns_addr,
            ns_script,
            ns_version,
