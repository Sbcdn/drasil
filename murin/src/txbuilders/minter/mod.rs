...
use clib::{NativeScript,NativeScripts,ScriptPubkey,ScriptAll};
...
impl MinterTxData {
    pub fn new(
        ...
        MinterTxData { mint_tokens, receiver_stake_addr, receiver_payment_addr, mint_metadata, auto_mint, fee_addr, fee, contract_id }
    }
...
impl ToString for MinterTxData {
    fn to_string(&self) -> String {
        ...
        ret.push_str(&(serde_json::to_string(&self.get_metadata()).expect("ERROR: Could not serialize metadata string")+"|"));
...
impl core::str::FromStr for MinterTxData {
    ...
    fn from_str(src: &str) -> std::result::Result<Self, Self::Err> {
        ...
        if slice.len() == 8 { 
            ...
            // restore fee
            debug!("restore fee");
            let metadata : Cip25Metadata = serde_json::from_str(slice[3]).expect("ERORR: Could not deserialize metadata from string");
            ...
            Err(
                MurinError::new(
                    &format!("Error the provided string '{}' cannot be parsed into 'RWDTxData' ",src).to_string()
                )
            )
        }
    }
...
pub fn make_mint_metadata_from_json(raw_metadata: &Cip25Metadata, tokens: Vec::<TokenAsset>, policy_id: clib::PolicyID) -> std::result::Result<clib::metadata::GeneralTransactionMetadata,MurinError> {
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        metamap.insert_str("version",&clib::metadata::TransactionMetadatum::new_text(raw_metadata.version.clone())?)?;
        ...
        if let Some(other) = &raw_metadata.other {
            for o in other.clone() {
                if o.value.len() >= 1 {
                    ...
                    metamap.insert_str(&o.key, &clib::metadata::TransactionMetadatum::new_list(&olist))?;
                } else {
                    metamap.insert_str(&o.key, &clib::metadata::TransactionMetadatum::new_text(o.value[0].clone())?)?;
        ...
        toplevel_metadata.insert(
            &cutils::to_bignum(721u64),
            &metadata
        );
...
pub fn make_721_asset_entry(asset: &AssetMetadata, assetmap: &mut clib::metadata::MetadataMap) -> std::result::Result<(),MurinError> {
    pub use clib::metadata::*;
    let mut asset_metadata = MetadataMap::new();
    if let Some(name) = &asset.name {
        asset_metadata.insert_str("name", &clib::metadata::TransactionMetadatum::new_text(name.clone())?)?;
    }
    if let Some(image_url) = &asset.image_url {
        asset_metadata.insert_str("image", &clib::metadata::TransactionMetadatum::new_text(image_url.clone())?)?;
        asset_metadata.insert_str("mediaType", &clib::metadata::TransactionMetadatum::new_text(
                asset.media_type.clone().expect("If an image url is provided a mediaType is mandatory").clone())?)?;
    }
    ...
    if let Some(descritpion) = &asset.descritpion {
        ...
        asset_metadata.insert_str("description", &clib::metadata::TransactionMetadatum::new_list(&desc_array))?;
    }
    ...
    if let Some(files) = &asset.files {
        ...
        for f in files.clone() {
            ...
            filemap.insert_str("name", &clib::metadata::TransactionMetadatum::new_text(f.name)?)?;
            filemap.insert_str("mediaType", &clib::metadata::TransactionMetadatum::new_text(f.media_type)?)?;
            let mut filelist = MetadataList::new();
            for s in f.src {
                filelist.add(&clib::metadata::TransactionMetadatum::new_text(s)?)
            }
            filemap.insert_str("src", &clib::metadata::TransactionMetadatum::new_list(&filelist))?;
            //Other
            if let Some(other) = f.other {
                ...
                for o in other {
                    if o.value.len() >= 1 {
                        ...
                        filemap.insert_str(&o.key, &clib::metadata::TransactionMetadatum::new_list(&filelist))?;
                    } else {
                        filemap.insert_str(&o.key, &clib::metadata::TransactionMetadatum::new_text(o.value[0].clone())?)?;
        ...
        asset_metadata.insert_str("files",&clib::metadata::TransactionMetadatum::new_list(&mfiles))?;
    }
    ...
    if let Some(other) = &asset.other {
        for o in other.clone() {
            if o.value.len() >= 1 {
                ...
                asset_metadata.insert_str(&o.key, &clib::metadata::TransactionMetadatum::new_list(&olist))?;
            } else {
                asset_metadata.insert_str(&o.key, &clib::metadata::TransactionMetadatum::new_text(o.value[0].clone())?)?;
...
pub fn create_onshot_policy(pub_key_hash : &Ed25519KeyHash, current_slot : u64 ) -> (NativeScript, ScriptHash) {
    let mut native_scripts = NativeScripts::new();
    native_scripts.add(&NativeScript::new_script_pubkey(&ScriptPubkey::new(pub_key_hash)));

    let slot = cutils::to_bignum(current_slot+1800);
    native_scripts.add(&NativeScript::new_timelock_expiry(&clib::TimelockExpiry::new_timelockexpiry(&slot)));
