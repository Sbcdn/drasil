...
pub fn store_raw_tx(
    ...
    let _response : () = redis::cmd("HMSET").arg(&key.id).arg(&items).query(&mut con)?;
...
pub fn read_raw_tx(
    key: &String 
) -> Result<RawTx,MurinError> {
    let mut con = redis_txmind_connection()?;

    let response : HashMap<String,String> = redis::cmd("HGETALL").arg(key).query(&mut con)?;
    let err = MurinError::new(&format!("Error in decoding Redis Data for 'RawTx' : {:?}",response).to_string());
    ...
    Ok(RawTx::new(
        ...
        &response.get("contractversion").ok_or(&err)?.parse::<f32>()?,
    ))
}