...
use drasil_murin::{calc_min_ada_for_utxo, find_token_utxos_na, wallet, MurinError};

pub type Result<T> = std::result::Result<T, MurinError>;

pub async fn optimize(addr: String, uid: i64, cid: i64) -> Result<()> {
    log::debug!("Try to connect to dbsync...");
    let contract_utxos = drasil_mimir::get_address_utxos(&addr).map_err(|e| e.to_string())?;
    ...
    let contract =
        drasil_hugin::TBContracts::get_contract_uid_cid(uid, cid).map_err(|e| e.to_string())?;
...
fn finalize_tx(
    ...
    let slot = drasil_mimir::get_slot(
...
pub async fn run_optimize(contract_id: i64) -> Result<()> {
    pretty_env_logger::init();
    println!("Start UTxO optimization");
    let contracts =
        drasil_hugin::TBContracts::get_all_active_rwd_contracts().map_err(|e| e.to_string())?;
    let contracts: Vec<drasil_hugin::TBContracts> = contracts
        .into_iter()
    ...
    'outer: for contract in contracts {
        ...
        let twl = drasil_gungnir::TokenWhitelist::get_whitelist().map_err(|e| e.to_string())?;
