...
impl Rewards {
    ...
    pub fn get_avail_specific_asset_reward(
        ...
        for r in result {
            ...
            match claim_sum {
                ...
                Err(e) => {
                    log::error!("FOR OBSERVATION: ERROR in claim search for NFT: {:?}",e.to_string());
                    res.push(r);
                }
            }
        ...
        match ((result.0/lovelace) - result.1.clone()).to_i64() {
            Some(dif) => {
                if claim_sum != result.1.to_i64().unwrap() {
                    return Err(RWDError::new("Error: Missmatch on claimed amount, please contact support!"))
                }
                Ok(dif - claim_request)
            },
            None => {
                Err(RWDError::new("Could not calculate available tokens for claim"))
            }
        }
    }
    ...
    pub fn update_rewards<'a>(
        ...
        use crate::schema::rewards::dsl::*;
        let contract = 
            diesel::update(rewards.filter(stake_addr.eq(stake_addr_in))
                                    .filter(fingerprint.eq(fingerprint_in))
                                    .filter(contract_id.eq(contract_id_in))
                                    .filter(user_id.eq(user_id_in)))
                .set((
    ...
    pub fn update_claimed<'a>(
        ...
        use crate::schema::rewards::dsl::*;
        let rwds = Self::get_rewards_per_token(&conn, &stake_addr_in, *contract_id_in, *user_id_in, &fingerprint_in)?;
        if rwds.len() == 1 {
        let total_claimed = rwds[0].tot_claimed.clone() + BigDecimal::from_u64(*claimed).unwrap();
        let contract = 
            diesel::update(rewards.filter(stake_addr.eq(stake_addr_in))
                                    .filter(fingerprint.eq(fingerprint_in))
                                    .filter(contract_id.eq(contract_id_in))
        ...
        Ok(contract)
            ...
            Err(RWDError::new(&format!("Could not find rewards for Stake Addr: {}, Contract-ID: {}, User-Id: {}", stake_addr_in,contract_id_in,user_id_in)))
        }
    }
...
impl Claimed {
    ...
    pub fn invalidate_claim<'a>(
        ...
        use crate::schema::claimed::dsl::*;
        let contract = 
            diesel::update(claimed.filter(stake_addr.eq(stake_addr_in))
                                    .filter(fingerprint.eq(fingerprint_in))
                                    .filter(contract_id.eq(contract_id_in))
                                    .filter(user_id.eq(user_id_in)))
                .set((
                    invalid.eq(invalid_in),
                    invalid_descr.eq(invalid_descr_in),
                ))
                .get_result::<Claimed>(conn)?;
    ...
    pub fn get_stat_count_all_tx_on_contr(
        ...
        let result = match fingerprint_in {
            Some(f) => {
                let result  =
                claimed.select(diesel::dsl::count_star())
    ...
    pub fn get_stat_count_all_tx_user(
         user_id_in : &i64,
    ) -> Result<i64,RWDError> {
        use crate::schema::claimed::dsl::*;
        let gconn = establish_connection()?;
...
impl TokenWhitelist{
    ...
    pub fn get_whitelist(
        ...
        let result = token_whitelist::table
                ...
                None => {
                    match tokenname_in {
                        Some(tn) => 
                                ...
                                .select((policy_id,tokenname.nullable(),fingerprint.nullable()))
                                .first::<TokenInfo>(&*conn)?,
                        None => return Err(RWDError::new("No tokenName and no fingerprint provided to retrieve tokeninfo"))
    ...
    pub fn update_twl<'a>(
        ...
        use crate::schema::token_whitelist::dsl::*;
        let twl = 
            diesel::update(token_whitelist.filter(fingerprint.eq(fingerprint_in))
                                    .filter(contract_id.eq(contract_id_in))
                                    .filter(user_id.eq(user_id_in)))
                .set((
    ...
    pub fn remove_twl<'a>(
        ...
        let result = 
            diesel::delete(
                token_whitelist.filter(fingerprint.eq(fingerprint_in))
                    .filter(contract_id.eq(contract_id_in))
                    .filter(user_id.eq(user_id_in)))
                    .get_result::<TokenWhitelist>(&conn)?;
    ...
    pub fn get_pools(
        ...
        resp.extend(result.iter().map(|n| GPools::from_str(n).expect("Could not convert string to GPools")));
    ...
    pub fn add_pools<'a>(
        ...
        let conn = establish_connection()?;
        let twl = TokenWhitelist::get_whitelist_entry(&conn, &fingerprint_in, contract_id_in.clone(), user_id_in.clone())?;
        
        let mut old_pools = Vec::<GPools>::new();
        old_pools.extend(twl[0].pools.iter().map(|n| GPools::from_str(n).expect("Could not convert string to GPools")));
...
impl AirDropWhitelist {
    ...
    pub fn use_awl<'a>(
        ...
        use crate::schema::airdrop_whitelist::dsl::*;
        let twl = 
            diesel::update(airdrop_whitelist
                                    .filter(contract_id.eq(contract_id_in))
                                    .filter(user_id.eq(user_id_in)))
                .set(
                    reward_created.eq(true)
                )
                .get_result::<AirDropWhitelist>(conn)?;
    ...
    pub fn remove_awl<'a>(
        ...
        let result = 
            diesel::delete(airdrop_whitelist
                                    .filter(contract_id.eq(contract_id_in))
                                    .filter(user_id.eq(user_id_in)))
...
impl AirDropParameter {
    ...
    pub fn remove_airdrop_parameter<'a>(
        ...
        use crate::schema::airdrop_parameter::dsl::*;
        let result = 
            diesel::delete(airdrop_parameter
                            .find(id_in))
                            .execute(conn)?;
...
impl WlAddresses {
    ...
    pub fn remove_wladdress<'a>(
        conn                   : &PgConnection,
        id_in                  : i64,
    ) -> Result<usize,RWDError> {
        use crate::schema::wladdresses::dsl::*;
        let result = 
            diesel::delete(wladdresses
                            .find(id_in))
                            .execute(conn)?;
...
impl Whitelist {
    ...
    pub fn remove_wl<'a>(
        ...
        let result = 
            diesel::delete(whitelist::table
                            .filter(whitelist::id.eq(wl_in))
            )
            .execute(conn)?;
        Ok(result)
    }
...
impl MintProject {
    ...
    pub fn remove_mintproject<'a>(
        conn                : &PgConnection,
        id_in               : &'a i64,
    ) -> Result<usize,RWDError> {
        let result = 
            diesel::delete(mint_projects::table
                            .find(id_in)
            )
            .execute(conn)?;
...
impl Nft {
    ...
    pub fn set_nft_minted<'a>(
        ...
        let result = 
            diesel::update(nft_table::table
                            .find((id_in,assetnameb))
                            ).set(
                                (
        ...
        let result = 
            diesel::update(nft_table::table
                            .find((id_in,assetnameb))
                                    ...
                                    nft_table::payment_addr.eq(Some(payment_addr_in)) 
                            )
            .get_result::<Nft>(conn)?;
        ...
        let result = 
            diesel::update(nft_table::table
                            .find((id_in,assetnameb))
                                    ...
                                    nft_table::ipfs_hash.eq(Some(ipfs)) 
                            )
            .get_result::<Nft>(conn)?;
        ...
        let result = 
            diesel::update(nft_table::table
                            .find((id_in,assetnameb))
                                    ...
                                    nft_table::metadata.eq(metadata) 
                            )
            .get_result::<Nft>(conn)?;
