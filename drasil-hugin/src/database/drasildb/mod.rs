...
use diesel::prelude::*;
use drasil_gungnir::{BigDecimal, FromPrimitive, ToPrimitive};
...
pub struct CaValue {
    pub ada_amount: drasil_gungnir::BigDecimal,
...
impl CaValue {
    ...
    pub fn into_cvalue(&self) -> Result<drasil_murin::clib::utils::Value, SystemDBError> {
        let coin = drasil_murin::clib::utils::to_bignum(self.ada_amount.to_u64().unwrap());
        let mut value = drasil_murin::clib::utils::Value::new(&coin);
        let mut ma = drasil_murin::clib::MultiAsset::new();
        if !self.token.is_empty() {
            for t in &self.token {
                let mut asset = drasil_murin::clib::Assets::new();
