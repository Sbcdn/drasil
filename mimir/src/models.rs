...
use diesel::prelude::*;
use diesel::sql_types::{Float8, Array, Int4, Jsonb, Numeric, BigInt};
...
impl UnspentUtxo {
    ...
    pub fn to_txuo(&self, conn: &PgConnection) -> Result<murin::TransactionUnspentOutput, murin::MurinError> {
        let input = murin::TransactionInput::new(&murin::crypto::TransactionHash::from_bytes(self.hash.clone())?, self.index as u32);
        let address = murin::address::Address::from_bech32(&self.address)?;
        let coin = match self.value.to_u64() {
            Some(c) => c,
            None    => {return Err(murin::MurinError::new(&format!("Could not determinte Lovelace on Utxo:  {:?}#{:?}",input,self.index).to_string())) }
        };

        let mut amount = murin::utils::Value::new(&murin::utils::to_bignum(coin));

        let tokens = crate::api::get_utxo_tokens(conn, self.id)?;
        let mut ma = murin::clib::MultiAsset::new();
        for tok in tokens {
            match ma.get(&murin::PolicyID::from_bytes(tok.policy.clone())?) {
                Some(mut asset) => {
                    asset.insert(&murin::AssetName::new(tok.name)?, &murin::utils::to_bignum(tok.quantity.to_u64().unwrap()));
                    ma.insert(&murin::PolicyID::from_bytes(tok.policy)?, &asset);
                    //println!("I ADDED AN EXISTING POLICY IDS ASSET : {:?}",ma);
                }
                None => {
                    let mut a = murin::clib::Assets::new();
                    a.insert(&murin::AssetName::new(tok.name)?, &murin::utils::to_bignum(tok.quantity.to_u64().unwrap()));
                    ma.insert(&murin::PolicyID::from_bytes(tok.policy)?, &a);
                }
            }
        }
        if ma.len() > 0 {
        ...
        let output = murin::TransactionOutput::new(&address, &amount);


        Ok(murin::TransactionUnspentOutput::new(&input, &output))
    }
}
