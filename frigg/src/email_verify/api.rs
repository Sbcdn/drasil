...
pub async fn invite(body: TBEmailVerificationTokenMessage) -> crate::WebResult<impl warp::Reply>  {
    let token = match TBEmailVerificationToken::create(body.clone()) {
        Ok(t) => t,
        Err(_) => {
            return Err(warp::reject::custom(Error::Custom("Could not create verification token".to_string())))
        }
    };
    ...
    Ok(warp::reply::with_status(
        warp::reply::json(
            &Msg::new(format!("User successfully registered, please verify e-mail address to proceed. E-Mail: {}", body.email))),
...
pub async fn verify(body: RegistrationMessage) -> crate::WebResult<impl warp::Reply> {
    ...
    let token = TBEmailVerificationToken::find(&token_id)
        .map_err(|_| 
                Error::Custom("Invalid token".to_string())
        )?;

    if token.email != body.email {
        return Err(warp::reject::custom(Error::Custom("Invalid token".to_string())));
    }
    ...
    let _ = match hugin::drasildb::TBDrasilUser::verify_email(&token.email) {
        Ok(u)  => u,
        Err(_) => return Err(warp::reject::custom(Error::Custom("Could not update user please retry the verification process".to_string())))
    };

    Ok(warp::reply::with_status(warp::reply::json(&Msg::new("Successfully verified".to_string())),warp::http::StatusCode::OK))
}