...
async fn main() {
    ...
    let user_route = warp::path("use")
        .and(with_auth(Role::StandardUser));


      // get DrasilUser profile
    let user_get_profile = user_route.clone()
    ...
    let enterprise_route = warp::path("ent")
        .and(with_auth(Role::EnterpriseUser));
    
    let enterprise_get = enterprise_route.clone()
        .and(warp::get());

    let enterprise_post = enterprise_route.clone()
        .and(warp::post());
    
    let enterprise_create_api_token = 
        enterprise_get.clone()
        .and(warp::get())
        .and(warp::path("api"))
        .and(warp::path("cr"))
    ...
    let enterprise_get_contracts = enterprise_get.clone()
        .and(warp::path("rwd"))
        .and(warp::path("contr"))
        .and_then(handler::rwd::enterprise_get_rwd_contracts_handler);

    // get set pool in a contract
    let enterprise_get_pools = enterprise_get.clone()
        .and(warp::path("sprwc"))
        .and(warp::path("pools"))
    ...
    // get set pool in a contract
    let enterprise_get_user_tx = enterprise_get.clone()
        .and(warp::path("ms"))
        .and(warp::path("stats"))
        .and(warp::path("sprwc"))
        .and(warp::path("tx"))
    ...
    // get set pool in a contract
    let enterprise_get_contract_tokens = enterprise_get.clone()
        .and(warp::path("sprwc"))
    ...
    let enterprise_post_create_reward_contract = enterprise_post.clone()
        .and(warp::path("ms"))
        .and(warp::path("cr"))
        .and(warp::path("sprwc"))
        .and(warp::body::content_length_limit(100 * 1024).and(warp::body::json()))
    ...
    let enterprise_post_deprecate_reward_contract = enterprise_post.clone()
        .and(warp::path("ms"))
        .and(warp::path("depr"))
        .and(warp::path("sprwc"))
        .and(warp::body::content_length_limit(100 * 1024).and(warp::body::json()))
    ...
    let enterprise_post_add_token_sporwc = enterprise_post.clone()
        .and(warp::path("sprwc"))
        .and(warp::path("addt"))
        .and(warp::body::content_length_limit(100 * 1024).and(warp::body::json()))
    ...
    let enterprise_post_rm_token_sporwc = enterprise_post.clone()
        .and(warp::path("sprwc"))
        .and(warp::path("rmt"))
        .and(warp::body::content_length_limit(100 * 1024).and(warp::body::json()))
    ...
    let enterprise_post_add_pools = enterprise_post.clone()
        .and(warp::path("sprwc"))
    ...
    let enterprise_post_rm_pools = enterprise_post.clone()
        .and(warp::path("sprwc"))
    ...
    let pools = 
        enterprise_get_pools
        .or(enterprise_post_add_pools)
        .or(enterprise_post_rm_pools);

    let sporwc = 
        enterprise_post_create_reward_contract
    ...
    let retailer_route = warp::path("ret")
        .and(with_auth(Role::Retailer));

    let retailer_get = retailer_route.clone()
        .and(warp::get());

    let retailer_post = retailer_route.clone()
        .and(warp::post());

    // Reactivate a Depricated Reward Contract
    let enterprise_post_reactivate_reward_contract = retailer_post.clone()
        .and(warp::path("ms"))
        .and(warp::path("act"))
        .and(warp::path("sprwc"))
        .and(warp::body::content_length_limit(100 * 1024).and(warp::body::json()))
    ...
    let retailer = 
    enterprise_post_reactivate_reward_contract;
    ...
    match user {
        Ok(u) => {
            match u.email_verified {
                ...
                _ => {
                    Err(reject::custom(error::Error::EmailNotVerified))
                }
            }
        }
...
pub async fn register_handler(payload : RegisterRequest) -> WebResult<impl Reply> {
    let conn = hugin::drasildb::establish_connection().map_err(|_| error::Error::Custom("Could not establish database connection".to_string()))?;
    ...
    let email_body = hugin::database::TBEmailVerificationTokenMessage::new(Some(new_user.uname.clone()), &new_user.email);
...
pub async fn enterprise_post_handler(uid: String, param :String, json : String) -> WebResult<impl Reply> {
