...
impl UnspentUtxo {
    pub fn to_txuo(
        &self,
        conn: &PgConnection,
    ) -> Result<murin::TransactionUnspentOutput, murin::MurinError> {
        let input = murin::TransactionInput::new(
            &murin::crypto::TransactionHash::from_bytes(self.hash.clone())?,
            self.index as u32,
        );
        ...
        for tok in tokens {
            match ma.get(&murin::PolicyID::from_bytes(tok.policy.clone())?) {
                Some(mut asset) => {
                    asset.insert(
                        &murin::AssetName::new(tok.name)?,
                        &murin::utils::to_bignum(tok.quantity.to_u64().unwrap()),
                    );
