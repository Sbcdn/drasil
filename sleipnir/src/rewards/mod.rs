...
pub async fn create_contract(
    ...
    // create key1
    let root_key1 : clib::crypto::Bip32PrivateKey = clib::crypto::Bip32PrivateKey::generate_ed25519_bip32()?;
    ...
    let _vkey1 = "5840".to_string()+&((hex::encode(ac1_publick_key.as_bytes()))+&hex::encode(ac1_chaincode.clone())); // .vkey
    let _skey1 = "5880".to_string()+&( hex::encode(ac1_private_key.as_bytes()) + &hex::encode(ac1_publick_key.as_bytes())+ &hex::encode(ac1_chaincode)) ; // .vkey

    // create key2
    let root_key2 : clib::crypto::Bip32PrivateKey = clib::crypto::Bip32PrivateKey::generate_ed25519_bip32()?;
    ...
    let _vkey2 = "5840".to_string()+&((hex::encode(ac2_publick_key.as_bytes()))+&hex::encode(ac2_chaincode.clone())); // .vkey
    let _skey2 = "5880".to_string()+&( hex::encode(ac2_private_key.as_bytes()) + &hex::encode(ac2_publick_key.as_bytes())+ &hex::encode(ac2_chaincode)) ; // .vkey
    
    
    let mut native_scripts = NativeScripts::new();
    native_scripts.add(&NativeScript::new_script_pubkey(&ScriptPubkey::new(&ac1_public_key_hash)));
    native_scripts.add(&NativeScript::new_script_pubkey(&ScriptPubkey::new(&ac2_public_key_hash)));
    ...
    let script_address_e = clib::address::EnterpriseAddress::new(net_bytes, &stake_creds).to_address();
    let sc_address_bech32 = script_address_e.to_bech32(None)?;
    let d = &format!("RWD Multi Signature Native Script user: {:?}",user_id.clone())[..];
    ...
    let _kl = TBMultiSigLoc::create_multisig_keyloc(&user_id,  &contract_id, &0.1, &sc_address_bech32, Some(&sc_address_bech32), fee.as_ref(), &pvks, &false).await?;
...
pub async fn get_rwd_contracts_for_user(
    user_id             : i64, 
) -> Result<serde_json::Value,SleipnirError> {   
    use chrono::Datelike;
    use chrono::TimeZone;
    let contracts = hugin::TBContracts::get_all_contracts_for_user_typed(
        user_id,
        "sporwc".to_string(),
    )?;

    let current_date = chrono::Utc::now();
    let first : chrono::NaiveDateTime = chrono::NaiveDate::from_ymd(current_date.year(), current_date.month(), 1).and_hms(00, 00, 00);
    ...
    resp.extend(contracts.iter().map(|c| {
        let all_tx = gungnir::Claimed::get_stat_count_all_tx_on_contr(c.contract_id, user_id, None).unwrap();
        let month_tx = gungnir::Claimed::get_stat_count_period_tx_contr_token(None, c.contract_id, user_id, date_time, current_date).unwrap();
...
pub async fn create_token_whitelisting(
    ...
    if start_epoch < current_epoch {
        log::error!("Start epoch cannot be smaller as current epoch, current epoch set as start epoch...");
    ...
    if let Some (ps) = pools {
        for pool in &ps {
            if !mimir::find_avail_pool(pool)? {
                return Err(SleipnirError::new(&format!("One of the pools is not existing or retired: {}",pool)))
            }
        }
        spools.extend(ps.iter().map(|p| gungnir::GPools{ pool_id :  p.clone(), first_valid_epoch:current_epoch })) ;
    }
    ...
    match gungnir::Calculationmode::from_str(&mode)? {
        ...
        _  => {
            return Err(SleipnirError::new(&format!("Calculation Mode is invalid: {:?}",mode)))
        }
    }

    log::debug!("Process modificator equiation...");
    if let Some(_) = modificator_equ {
        return Err(SleipnirError::new(&format!("Modificator equation not supported yet")))
    }
...
pub async fn remove_token_whitelisting(
    ...
    let resp = gungnir::TokenWhitelist::remove_twl(
        &fingerprint,
        &(contract_id),
        &(user_id),
    )?;
...
pub async fn get_pools(
    ...
    let resp = gungnir::TokenWhitelist::get_pools(
        &fingerprint,
        &(contract_id),
        &(user_id),
    )?;
...
pub async fn add_pools(
    ...
    let current_epoch = mimir::get_epoch(&mconn)? as i64;
    let mut spools = Vec::<gungnir::GPools>::new();
    spools.extend(pools.iter().map(|p| gungnir::GPools{ pool_id :  p.clone(), first_valid_epoch:current_epoch })) ;
 
    let resp = gungnir::TokenWhitelist::add_pools(
        &fingerprint,
        &(contract_id),
        &(user_id),
        &spools
...
pub async fn rm_pools(
    ...
    spools.extend(pools.iter().map(|p| gungnir::GPools{ pool_id :  p.clone(), first_valid_epoch:0 })) ;

    let resp = gungnir::TokenWhitelist::remove_pools(
        &fingerprint,
        &(contract_id),
        &(user_id),
        &spools
...
pub async fn get_user_txs(
    ...
    if let Some(date) = from {
        from_t = chrono::DateTime::<Utc>::from_utc(NaiveDateTime::parse_from_str(&date, "%Y-%m-%d %H:%M:%S")?,Utc);
    }

    let mut to_t = chrono::Utc::now() ;
    if let Some(date) = to.clone() {
        to_t = chrono::DateTime::<Utc>::from_utc(NaiveDateTime::parse_from_str(&date, "%Y-%m-%d %H:%M:%S")?,Utc);
    }

    let mut resp = 0;
    match to {
        Some(_) => {
            let resp1 = gungnir::Claimed::get_stat_count_period_tx_user(
                &user,
                &from_t,
                &to_t
            );
        ...
        None => {
            resp = gungnir::Claimed::get_stat_count_all_tx_user(
                &user
            )?;
        }
    }
...
pub async fn get_tokens_from_contract(
    ...
    let resp = gungnir::TokenWhitelist::get_rwd_contract_tokens(
        contract_id,
        user_id,
    )?;
