...
pub async fn create_policy_script(
    ...
    if let Some(addr) = mint_signer.clone() {
        match clib::address::BaseAddress::from_address(&addr) {
            Some(baddr) => {
                native_scripts.add(&NativeScript::new_script_pubkey(&ScriptPubkey::new(&baddr.payment_cred().to_keyhash().unwrap())));
            },
            None => {
                return Err(SleipnirError::new("Provided Wallet Signature Address is invalid"));
            }
        } 
    };
    ...
    if let Some(date) = time_constraint {
        ...
        native_scripts.add(&NativeScript::new_timelock_expiry(&clib::TimelockExpiry::new_timelockexpiry(&murin::clib::utils::to_bignum(slot))));
    }
