...
#[structopt(name = "create minting policy", about = "Creates a minting policy contract")]
struct Opt {
    ...
    #[structopt(short, long, about="Optional: Wallet Bech32-Address which needs to sign the mint")]
    signer: Option<String>,

    #[structopt(short, long, about="Optionla: Date until for timelocking the policy-id")]
...
fn main() -> Result<(),SleipnirError> {
    ...
    if let Some(date) = opt.timelock {
        vd = Some(chrono::DateTime::<Utc>::from_utc(NaiveDateTime::parse_from_str(&date, "%Y-%m-%d %H:%M:%S")?,Utc));
    }
    ...
    sleipnir::minting::create_policy_script(
        network, 
        opt.user, 
        signer, 
        vd, 
