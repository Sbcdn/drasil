...
pub async fn create_mintproject(data: &CreateMintProj) -> Result<MintProject, SleipnirError> {
    ...
    let mint_contract =
        drasil_hugin::TBContracts::get_contract_uid_cid(data.user_id.unwrap(), policy_script_id)?;
    ...
    for result in rdr.records() {
        ...
        if let Ok(record) = result {
            for n in record.iter() {
                ...
                let assets = drasil_murin::minter::AssetMetadata::from_json(n)?;
...
pub async fn import_from_asset_metadata(
    ...
    let mint_contract =
        drasil_hugin::TBContracts::get_contract_uid_cid(user_id, mint_project.mint_contract_id)?;
