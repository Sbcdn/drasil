...
use drasil_hugin::TBCaPayment;
use drasil_murin::utils::from_bignum;

pub async fn create_rev_payout(
    user_id: i64,
    contract_id: i64,
    pw: String,
) -> Result<drasil_hugin::TBCaPayment, SleipnirError> {
    let contract = drasil_hugin::TBContracts::get_contract_uid_cid(user_id, contract_id)?;
    let address_utxos = drasil_mimir::get_address_utxos(&contract.address)?;

    let total_value = address_utxos.calc_total_value()?;
    let contract_lqdty = contract.get_contract_liquidity();
    let payout_value = drasil_hugin::CaValue::new(
        drasil_murin::utils::from_bignum(&total_value.coin().checked_sub(&contract_lqdty)?),
        vec![],
    );
    Ok(drasil_hugin::TBCaPayment::create(&user_id, &contract_id, &payout_value, &pw).await?)
}
...
pub async fn create_custom_payout(
    ...
    let payout_value = drasil_hugin::CaValue::new(ada_value as u64, tokens);
...
impl DispCaPayment {
    pub fn from_cap(caps: Vec<drasil_hugin::TBCaPayment>) -> Vec<Self> {
...
pub async fn show_payouts(user_id: i64) -> Result<Vec<DispCaPayment>, SleipnirError> {
    let caps = drasil_hugin::TBCaPayment::find_all(&user_id)
        .map_err(|e| SleipnirError::new(&e.to_string()))?;
    Ok(DispCaPayment::from_cap(caps))
}

pub async fn find_payout(user_id: i64, po_id: i64) -> Result<TBCaPayment, SleipnirError> {
    let caps =
        drasil_hugin::TBCaPayment::find(&po_id).map_err(|e| SleipnirError::new(&e.to_string()))?;
...
pub async fn approve_payout(
    ...
    let user = drasil_hugin::TBDrasilUser::get_user_by_user_id(user_id)?;

    let msg = drasil_hugin::TBCaPaymentHash::find_by_payid(payout_id)?[0]
        .payment_hash
        .clone();
    let payment = drasil_hugin::TBCaPayment::find(payout_id)?;
