...
use murin::{TxData, address::Address};
...
impl ContractType {
    pub const CONTRTYPES : [Self; 4] = [Self::MarketPlace, Self::NftShop, Self::NftMinter, Self::TokenMinter];
}
...
impl FromStr for ContractType {
    ...
    fn from_str(src: &str) -> Result<Self,Self::Err> {
        match src {
            ...
            _             => Err(Error::new( std::io::ErrorKind::InvalidData,format!("Contract Type {} does not exist",src))),
        }
    }
}
...
impl FromStr for MarketplaceActions {
    ...
    fn from_str(src: &str) -> Result<Self,Self::Err> {
        match src {
            ...
            _         => Err(Error::new( std::io::ErrorKind::InvalidData,format!("Marketplace Action {} does not exist",src))),
        }
    }
}
...
impl FromStr for MultiSigType {
    ...
    fn from_str(src: &str) -> Result<Self, Self::Err> {
        match src {
            ...
            _                   => Err(Error::new( std::io::ErrorKind::InvalidData,format!("Transaction Type {} does not exist",src))),
        }
    }
}
...
impl FromStr for ContractAction {
    ...
    fn from_str(src: &str) -> Result<Self, Self::Err> {
        match src {
            ...
            "cancel"  => Ok(ContractAction::MarketplaceActions(MarketplaceActions::Cancel)),
            "update"  => Ok(ContractAction::MarketplaceActions(MarketplaceActions::Update)),
            _             => Err(Error::new( std::io::ErrorKind::InvalidData,format!("ContractAction '{}' does not exist",src))),
        }
    }
} 
...
impl MinterToken {
    ...
    pub fn for_all_into_mintasset(ut : &Vec::<MinterToken>) -> Result<Vec::<murin::txbuilders::MintTokenAsset>,murin::MurinError> {
...
impl Token {
    ...
    pub fn for_all_into_asset(ut : &Vec::<Token>) -> Result<Vec::<murin::txbuilders::TokenAsset>,murin::MurinError> {
...
impl TransactionPattern {
    ...
    pub fn collateral(&self) -> Option<String> {
        match &self.collateral {
            Some(col) => {
                if col.len() > 0 { Some(col[0].clone()) } else { None }
            },
            None => None,
        }
    }
    ...
    pub async fn into_txdata(&self) -> Result<murin::txbuilders::TxData, murin::error::MurinError> {
        let inputs = match self.inputs() {
            None => {return Err(murin::error::MurinError::new("Cannot build transaction data, no inputs provided"))},
            Some(data) => data,
        };
        ...
        let mut txd = TxData::new(
            ...
            saddr,
            murin::wallet::get_transaction_unspent_outputs(inputs.as_ref(), self.collateral().as_ref(), self.excludes().as_ref()).await?, 
        ...
        if let Some(collateral) = self.collateral() {
            txd.set_collateral(
                murin::wallet::get_transaction_unspent_output(&collateral).await?
            )
        }
...
impl ScriptSpecParams { 
    ...
    pub async fn into_mp(&self, avail_inputs : murin::TransactionUnspentOutputs) -> Result<murin::txbuilders::marketplace::MpTxData, murin::error::MurinError> {
        ...
        match self {
            ScriptSpecParams::Marketplace {
                ...
                let token_utxos = murin::txbuilders::find_token_utxos(avail_inputs,assets.clone()).await?;

                // Transform tokens to TokenAsset (flattend Value)
                
                let mut mptx = MpTxData::new(
                    assets,
                    token_utxos,
                    *selling_price,
                );
                ...
                if let Some(royaddr) = royalties_addr {
                    mptx.set_royalties_address(
                        murin::decode_addr(&royaddr).await?
                    );
                }

                if let Some(royrate) = royalties_rate {
                    mptx.set_royalties_rate(
                        *royrate
                    );
                }
            ...
            _ => {
                Err(MurinError::new("provided wrong specfic paramter for this contract"))
            }
        }  
    }

    pub async fn into_rwd(&self) -> Result<murin::txbuilders::rwdist::RWDTxData, murin::error::MurinError> {
    ...
    pub async fn into_mintdata(&self) -> Result<murin::txbuilders::minter::MinterTxData, murin::error::MurinError> {
        ...
        match self {
            ScriptSpecParams::NftMinter {
                ...
                let stake_addr = match receiver_stake_addr {
                    Some(addr) => {
                        Some(murin::decode_addr(&addr).await?)
                    },
                    None => None
                };
    ...
    pub async fn into_stake_delegation(&self) -> Result<murin::txbuilders::delegation::DelegTxData, murin::error::MurinError> {
        ...
        match self {
            ...
            _ => {
                Err(MurinError::new("provided wrong specfic paramter for this transaction"))
            }
        }
    }
...
impl FromStr for WalletType {
    ...
    fn from_str(src: &str) -> Result<Self, Self::Err> {
        match src {
            ...
            _             => Err(Error::new( std::io::ErrorKind::InvalidData,format!("Wallet '{}' not supportet or wrong typed input",src))),
        }
    }
} 
