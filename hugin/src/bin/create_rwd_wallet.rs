...
#################################################################################
*/
use murin::clib::{NativeScript, ScriptPubkey, NativeScripts, ScriptAll, crypto::Bip32PrivateKey};
use murin::*;
...
struct Opt {
    ...
    #[structopt(short, long, about="Optional fixed fee for a transaction for using the service in lovelace")]
    fee: Option<i64>,
...
async fn main() -> Result<(),MurinError> {
    ...
    */
    ...
    let d = &format!("RWD Multi Signature Native Script user: {:?}",opt.user.clone())[..];
    ...
    for i in 0..rwd_script.get_required_signers().len() {
        println!("Required Signer {}: {:?}",i,hex::encode(rwd_script.get_required_signers().get(i).to_bytes()));
    }
    ...
    let _ = TBMultiSigLoc::create_multisig_keyloc(&opt.user,  &opt.contract_id, &opt.version, &script_address_e.to_bech32(None)? ,opt.wallet.as_ref(), opt.fee.as_ref(), &pvks, &false).await?;
