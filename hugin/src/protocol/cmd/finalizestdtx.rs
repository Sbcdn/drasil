...
impl IntoFrame for FinalizeStdTx {
    fn into_frame(self) -> Frame {
        ...
        let txtype_b = bc::DefaultOptions::new().with_varint_encoding().serialize(&self.txtype).unwrap();
    ...
    pub async fn apply(self, dst: &mut Connection) -> crate::Result<()> {
        ...
        match self.txtype {
            StdTxType::DelegateStake => {
                if let Err(e) = murin::delegation::DelegTxData::from_str(raw_tx.get_tx_specific_rawdata()) {
            ...
            _ => {
                return Err(CmdError::Custom{str:format!("ERROR MultiSigType does not exist: '{:?}'",self.txtype)}.into());
            }
