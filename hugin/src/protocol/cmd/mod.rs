...
impl Command {
    ...
    pub fn from_frame(frame: Frame) -> crate::Result<Command> {
        ...
        let command : Command = match &command_name[..] {
            //Build Contract
            "bct" => {
                Command::BuildContract(BuildContract::parse_frames(&mut parse)?)
            },
            //Build MultiSig
            "bms" => {
                Command::BuildMultiSig(BuildMultiSig::parse_frames(&mut parse)?)
            },
            //Finalize Contract
            "stx" => {
                Command::BuildStdTx(BuildStdTx::parse_frames(&mut parse)?)
            },
            //Finalize Contract
            "fct" => {
                Command::FinalizeContract(FinalizeContract::parse_frames(&mut parse)?)
            },
            //Finalize MultiSig
            "fms" => {
                Command::FinalizeMultiSig(FinalizeMultiSig::parse_frames(&mut parse)?)
            },
            //Finalize MultiSig
            "ftx" => {
                Command::FinalizeStdTx(FinalizeStdTx::parse_frames(&mut parse)?)
            },
            //GetPubKey
            "vus" => {
                Command::VerifyUser(VerifyUser::parse_frames(&mut parse)?)
            },
            //GetStakeKey
            "gsk" => {
...
async fn check_txpattern(txp : &TransactionPattern) -> crate::Result<()> {
    ...
    if !txp.outputs().is_none() {
        return Err(CmdError::Custom{str:"ERROR outputs must be None for this contract type".to_string()}.into());
    }
    
    if  (txp.collateral().is_some() && //txp.collateral().is_none() ||
         (hex::decode(txp.collateral().unwrap()).is_err() || txp.collateral().unwrap() == "")) {
        return Err(CmdError::Custom{str:"ERROR no collateral provided".to_string()}.into());
    }

    if txp.sending_wal_addrs().is_empty() {
        return Err(CmdError::Custom{str:"ERROR no wallet address provided".to_string()}.into());
    }
...
pub fn create_response (
    ...
    -> Result<crate::datamodel::hephadata::UnsignedTransaction,murin::MurinError>{
        ...
        let mut response = crate::datamodel::hephadata::UnsignedTransaction::new(Some(&bld_tx.get_tx_unsigned()),&tx_id);
