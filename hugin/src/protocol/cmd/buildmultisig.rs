...
#################################################################################
*/
use crate::{Parse, CmdError, TBContracts};
use crate::{Connection,Frame,IntoFrame};
use crate::datamodel::{MultiSigType,TransactionPattern,ScriptSpecParams,OneShotReturn};
...
impl BuildMultiSig {
    ...
    async fn handle_rewardclaim(&self) -> crate::Result<String> {
        ...
        match self.transaction_pattern().script().ok_or("ERROR: No specific contract data supplied")? {
            ScriptSpecParams::SpoRewardClaim{ 
                ...
                let err = Err(CmdError::Custom{str:format!("ERROR wrong data provided for script specific parameters: '{:?}'",self.transaction_pattern().script())}.into());
                if reward_tokens.is_empty()                                  ||
                    ...
                    murin::decode_addr(&recipient_payment_addr).await.is_err() {
                        ...
                        if murin::wallet::get_stake_address(&murin::decode_addr(&recipient_stake_addr).await?)? != 
                            murin::wallet::get_stake_address(&murin::decode_addr(&recipient_payment_addr).await?)? {
                                return err;
                            }
                    }
            },
            _ => {
                return Err(CmdError::Custom{str:format!("ERROR wrong data provided for '{:?}'",self.multisig_type())}.into());
            }
            
        }

        info!("create raw data...");
        let mut rwdtxd = self.transaction_pattern().script().unwrap().into_rwd().await?;
        ...
        if let None = contract {
            ...
            let contracts = crate::drasildb::TBContracts::get_all_contracts_for_user_typed(self.customer_id as i64, crate::MultiSigType::SpoRewardClaim.to_string())?;
            let mut all_tokens_available = Vec::<bool>::new();
            for c in contracts {
                all_tokens_available.extend( rwdtxd.get_reward_tokens().iter().map(
                    |t| {
                        let fingerprint =  murin::chelper::make_fingerprint(&hex::encode(t.0.to_bytes()), &hex::encode(t.1.name())).unwrap();
                        let has_wl = gungnir::TokenWhitelist::has_contract_valid_whitelisting(c.contract_id,self.customer_id as i64,&fingerprint).unwrap();
                        if has_wl {
                            // Check if whitelist token on contract also has available rewards
                            match gungnir::Rewards::get_available_rewards(
                                &gcon,
                                &gtxd.get_stake_address().to_bech32(None).expect("ERROR Could not construct bech32 address for stake address"),
                                &fingerprint,
            ...
            if contract.is_none() {
                return Err(CmdError::Custom{str:format!("Automatic selection failed, no suitable contract found.")}.into());
            }
        }
        let contract = contract.expect("Error: Could not unwrap contract");
        info!("Contract selected: UID: '{}', CID '{}'",self.customer_id, contract.contract_id);
        ...
        let vesting_whitelist = gungnir::TokenWhitelist::get_in_vesting_filtered_whitelist(contract.contract_id, self.customer_id as i64)?;
        ...
        for vt in vesting_whitelist {
            ...
            let assetname = match vt.tokenname {
                ...
                None => murin::clib::AssetName::new(b"".to_vec()).expect("Could not create emtpy tokenname")
            }; 
        ...
        if rwdtxd.get_reward_tokens().len() == 0 {
            return Err(CmdError::Custom{str:format!("The requested tokens are all still in a vesting period.")}.into());
        }
        ...
        for token in rwdtxd.get_reward_tokens() {
            ...
            if token.2.compare(&murin::clib::utils::to_bignum(0)) <= 0 {
                return Err(CmdError::Custom{str:format!("ERROR claiming amount '0' of a token is not allowed: '{:?}'",fingerprint)}.into());
            }
            info!("Try to get available rewards!");
            log::debug!("{:?}",contract);
            log::debug!("{:?}",fingerprint);
            log::debug!("{:?}",murin::clib::utils::from_bignum(&token.2) as i64);
            log::debug!("{:?}",gtxd.get_stake_address().to_bech32(None).expect("ERROR Could not construct bech32 address for stake address"));
            ...
            match gungnir::Rewards::get_available_rewards(
                ...
                murin::clib::utils::from_bignum(&token.2) as i64,
                 ...
                 i if i >= 0 => {
                    info!("User has enough tokens earned to claim: '{:?}'",fingerprint);
                },
                 _ => {
                    info!("Error in Rewards!");
                    return Err(CmdError::Custom{str:format!("ERROR user did not earned enough tokens to claim this amount: '{:?}'",token)}.into());
                 },
            }
        ...
        let keyloc = crate::drasildb::TBMultiSigLoc::get_multisig_keyloc(&drasildbcon, &contract.contract_id, &contract.user_id, &contract.version)?;
        ...
        if reward_wallet_utxos.is_empty() {
            ...
            return Err(CmdError::Custom{str:format!("The contract does not contain enough tokens to claim, please try again later")}.into());
        }
        ...
        */
        ...
        let ident = crate::encryption::mident(&contract.user_id, &contract.contract_id, &contract.version, &ns_addr);
        let pkvs = crate::encryption::decrypt_pkvs(keyloc.pvks, &ident).await?;
        info!("build transaction...");
        let bld_tx = murin::rwdist::build_rwd_multisig(&gtxd, &rwdtxd, &pkvs, &ns_addr, &ns_script, &ns_version, &apply_system_fee).await?;
    ...
    async fn handle_collection_mint(&self) -> crate::Result<String> {
                ...
                if murin::decode_addr(&receiver_payment_addr).await.is_err()
                    {
                        ...
                        if let Some(saddr) = receiver_stake_addr{ 
                            if murin::wallet::get_stake_address(&murin::decode_addr(&saddr).await?)? != 
                                murin::wallet::get_stake_address(&murin::decode_addr(&receiver_payment_addr).await?)? {
                                return err;
                            }
                        }
                    }
        ...
        let mut minttxd = self.transaction_pattern().script().unwrap().into_mintdata().await?;
        let mut gtxd = self.transaction_pattern().into_txdata().await?;
        
        log::debug!("Check contract...");
        let contract = determine_contract(gtxd.get_contract_id(), self.customer_id as i64)?.expect("Could not find valid contract");      
        ...
        if let Some(feeaddr) = keyloc.fee_wallet_addr {
            match murin::b_decode_addr(&feeaddr).await {
                ...
                Err(e) => {
                    return Err(CmdError::Custom{str:format!("ERROR could not decode fee address: '{:?}'", e.to_string())}.into());
                },
            };   
        };
        ...
        let gcon = gungnir::establish_connection()?;
        let mint_project = gungnir::MintProject::get_mintproject_by_uid_cid(&gcon, self.customer_id as i64, contract.contract_id)?;
        ...
        let policy_id = contract.policy_id.expect("Error: The provided contract is not eligable to mint, no policy ID");
        ...
        if mint_project.reward_minter {
            nfts_to_mint = gungnir::Nft::get_nft_by_payaddr(&gcon, mint_project.id, &payment_addr_bech32)?;
            nfts_to_mint.retain(|n| n.minted == false);
            if nfts_to_mint.len() <= 0 {
                return Err(CmdError::Custom{str:format!("ERROR No assets for this address available")}.into());
            }
            for nft in &nfts_to_mint {
                ...
                let rewards = gungnir::Rewards::get_avail_specific_asset_reward
                    (&gcon, &payment_addr_bech32, &murin::cip30::get_bech32_stake_address_from_str(&payment_addr_bech32)? ,&fingerprint, contract.contract_id, self.customer_id())?;
                if rewards.len() > 0 {
            ...
            let claimed = gungnir::Claimed::get_claims(&gcon, &murin::cip30::get_bech32_stake_address_from_str(&payment_addr_bech32)?, contract.contract_id, self.customer_id())?;
        ...
        minttxd.set_mint_tokens(convert_nfts_to_minter_token_asset(&nfts_to_mint,&policy_id)?);
        ...
        let metadata = murin::minter::Cip25Metadata{assets: metadataassets, other: None,  version: "1.0".to_string() };
        ...
        let dbsync = match mimir::establish_connection(){
            Ok(conn) => conn,
            Err(e)=> {
                return Err(CmdError::Custom{str:format!("ERROR could not connect to dbsync: '{:?}'", e.to_string())}.into());
            }
        };
        let slot = match mimir::get_slot(&dbsync) {
            Ok(s) => s,
            Err(e) => {
                return Err(CmdError::Custom{str:format!("ERROR could not determine current slot: '{:?}'", e.to_string())}.into());
            }
        };
        gtxd.set_current_slot(slot as u64);
        ...
        let bld_tx = murin::minter::build_mint_multisig(&gtxd, &minttxd, &pkvs, ns_addr.as_ref(), &ns_script, &ns_version, &apply_mint_fee).await?;
    ...
    async fn handle_onehshot_mint(&self) -> crate::Result<String> {
        ...
        let minttxd = self.transaction_pattern().script().unwrap().into_mintdata().await?;
        ...
        let bld_tx = match murin::minter::build_oneshot_mint::build_oneshot_mint_multisig(
            ...
            Ok(o) => o,
            Err(e) => {
                return Err(CmdError::Custom{str:format!("ERROR could not build transaction: '{:?}'", e.to_string())}.into());
            }
        };
    ...
    async fn handle_testrewards(&self) -> crate::Result<String> {
        ...
        if gtxd.get_network() != murin::clib::NetworkIdKind::Testnet {
            return Err(CmdError::Custom{str:format!("ERROR: this functions is just for testing")}.into());
        }
        log::info!("Randomize....");
        ...
        let mta1 : murin::txbuilders::MintTokenAsset = (None, murin::clib::AssetName::new("tFLZC".as_bytes().to_vec()).unwrap(), murin::clib::utils::to_bignum(138));
        let mta2 : murin::txbuilders::MintTokenAsset = (None, murin::clib::AssetName::new("tSIL".as_bytes().to_vec()).unwrap(), murin::clib::utils::to_bignum(142));
        let mta3 : murin::txbuilders::MintTokenAsset = (None, murin::clib::AssetName::new("tDRSL".as_bytes().to_vec()).unwrap(), murin::clib::utils::to_bignum(63));
        t1.push(mta1);
        t1.push(mta2);
        t1.push(mta3);
        ...
        let contract = crate::drasildb::TBContracts::get_contract_uid_cid(
            t_minter_user_id, t_minter_contract_id)?;

        let sporwc_flz =  crate::drasildb::TBContracts::get_contract_uid_cid(
            sporwc_user_id, sporwc_tcontract_id)?;
        ...
        let rnd_mintdata = murin::minter::MinterTxData::new(
           ...
           contract_address.clone(),//reward contract address
            serde_json::from_str(&metadataarray[i]).expect("ERROR: Could not deserialize metadata in testrewards method"),
            true, //auto_mint
            None,
            None,
        ...
        if let Some(saddr) = keyloc.fee_wallet_addr {
            match murin::b_decode_addr(&saddr).await {
                Ok(a) => minttxd.set_fee_addr(a),
                Err(e) => {
                    return Err(CmdError::Custom{str:format!("ERROR could not decode signers address: '{:?}'", e.to_string())}.into());
                },
            };
        ...
        debug!("RAWTX data: {:?}",tx);
        let used_utxos = tx.get_usedutxos().clone();
        let txh = murin::finalize_rwd(&hex::encode(&murin::clib::TransactionWitnessSet::new().to_bytes()), tx, pkvs).await?;
        murin::utxomngr::usedutxos::store_used_utxos(&txh, &murin::TransactionUnspentOutputs::from_hex(&used_utxos)?)?;
        ...
        let policy = hex::encode(murin::clib::NativeScript::from_bytes(hex::decode(contract.plutus).unwrap()).unwrap().hash().to_bytes());
        let fingerprint = murin::make_fingerprint( &policy,&tns[i])?;
        let rewards = gungnir::Rewards::get_rewards_per_token(&gconn, &gtxd.get_stake_address().to_bech32(None).unwrap(), sporwc_flz.contract_id as i64, sporwc_flz.user_id as i64, &fingerprint)?;
        ...
        if rewards.len() == 1 && rewards[0].fingerprint == fingerprint {
            let tot_earned = rewards[0].tot_earned.clone() + (gungnir::BigDecimal::from_u64(murin::clib::utils::from_bignum(&minttxd.get_mint_tokens()[0].2)).unwrap()*gungnir::BigDecimal::from_u64(1000000).unwrap());
        ...
        if rewards.len() == 0 {
            let tot_earned = gungnir::BigDecimal::from_u64(murin::clib::utils::from_bignum(&minttxd.get_mint_tokens()[0].2)).unwrap()*gungnir::BigDecimal::from_u64(1000000).unwrap();

            let stake_rwd = gungnir::Rewards::create_rewards(
                            &gconn, 
                            &gtxd.get_stake_address().to_bech32(None).unwrap(),
                            &gtxd.get_senders_address(None).expect("No payment address found").to_bech32(None).unwrap(), 
                            &fingerprint, 
    ...
    async fn handle_customer_payout(&self) -> crate::Result<String> {
        ...
        info!("create raw data...");
        let mut cpo_data = self.transaction_pattern().script().unwrap().into_cpo().await?;
        let mut gtxd = self.transaction_pattern().into_txdata().await?;
        ...
        let contract = crate::drasildb::TBContracts::get_contract_uid_cid(
            cpo_data.get_user_id(), cpo_data.get_contract_id())?;
...
pub fn determine_contract(contract_id : Option<u64>, customer_id : i64) -> Result<Option<crate::drasildb::TBContracts>,MurinError>{
    ...
    if let Some(contract_id) = contract_id {
        ...
        let tcontract = crate::drasildb::TBContracts::get_contract_uid_cid(u_customer_id, u_contract_id);
...
pub fn convert_nfts_to_minter_token_asset(nfts : &Vec::<gungnir::Nft>, policy_id : &String) ->  Result<Vec::<murin::MintTokenAsset> ,MurinError> {
