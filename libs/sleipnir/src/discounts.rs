...
pub async fn create_discount(params: DiscountParams) -> Result<serde_json::Value, SleipnirError> {
    let new = gungnir::Discount::create_discount(
        &params.user_id,
        &params.contract_id,
        &params.policy_id,
...
pub async fn get_discounts(cid: i64, uid: i64) -> Result<serde_json::Value, SleipnirError> {
    let resp = gungnir::Discount::get_discounts(cid, uid)?;

    Ok(json!(resp))
}

pub async fn remove_discount(params: DiscountParams) -> Result<serde_json::Value, SleipnirError> {
    let discounts = gungnir::Discount::get_discounts(params.contract_id, params.user_id)?;
    let discount: Vec<_> = discounts
        ...
        .collect();
    let resp = gungnir::Discount::remove_discount(&discount[0].id)?;
